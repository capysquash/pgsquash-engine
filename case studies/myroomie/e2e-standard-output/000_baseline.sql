...
DECLARE
  ...
  room_pref_count INTEGER;
BEGIN
  SELECT COUNT(*) INTO budget_count FROM property_options WHERE category = 'budget_range';
  SELECT COUNT(*) INTO comm_count FROM property_options WHERE category = 'communication_style';
  SELECT COUNT(*) INTO tenant_count FROM property_options WHERE category = 'tenant_preference';
  SELECT COUNT(*) INTO room_pref_count FROM property_options WHERE category = 'room_preference';
  RAISE NOTICE 'Migration 37 completed:';
  RAISE NOTICE '  - Budget ranges: % options', budget_count;
  RAISE NOTICE '  - Communication styles: % options', comm_count;
  RAISE NOTICE '  - Tenant preferences: % options', tenant_count;
  RAISE NOTICE '  - Room preferences: % options', room_pref_count;
END $$;
