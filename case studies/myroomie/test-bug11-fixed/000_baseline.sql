...
-- Generated at: 2025-11-06T19:55:28+02:00
...
CREATE VIEW properties_fairrent_ready AS
SELECT
  p.*,
  CASE
...
RETURNS TABLE (
  total_scored_properties BIGINT,
  avg_score NUMERIC,
  excellent_deals_count BIGINT,
  good_deals_count BIGINT,
  fair_deals_count BIGINT,
  overpriced_count BIGINT,
  avg_monthly_savings NUMERIC,
  total_active_scores BIGINT
) LANGUAGE plpgsql
SECURITY DEFINER VOLATILE
AS $$
BEGIN
...
) RETURNS TABLE (api_key_id UUID, api_key TEXT, key_prefix TEXT) LANGUAGE plpgsql VOLATILE AS $$
DECLARE
    v_api_key TEXT;
    v_key_hash TEXT;
    v_key_prefix TEXT;
...
) RETURNS TABLE (operation_result JSONB) LANGUAGE plpgsql VOLATILE AS $$
DECLARE
    v_result JSONB;
...
) RETURNS TABLE (success BOOLEAN, message TEXT, profile JSONB)  LANGUAGE plpgsql AS $$
DECLARE
    current_user_id TEXT;
