...
END $$; CREATE INDEX IF NOT EXISTS idx_conversations_participants ON conversations USING btree (participant_1_id, participant_2_id); CREATE INDEX IF NOT EXISTS idx_user_reviews_reviewee_id ON user_reviews USING btree (reviewee_id); CREATE INDEX IF NOT EXISTS idx_matches_users ON matches USING btree (user1_id, user2_id); CREATE INDEX IF NOT EXISTS idx_community_memberships_user ON community_memberships USING btree (user_id, community_id); CREATE INDEX IF NOT EXISTS idx_bookings_user_room ON bookings USING btree (user_id, room_id); CREATE INDEX IF NOT EXISTS idx_notifications_recipient_id ON notifications USING btree (recipient_id); CREATE INDEX IF NOT EXISTS idx_user_reviews_reviewer_id ON user_reviews USING btree (reviewer_id); CREATE INDEX IF NOT EXISTS idx_messages_sender ON messages USING btree (sender_id); CREATE INDEX IF NOT EXISTS idx_messages_conversation ON messages USING btree (conversation_id); CREATE POLICY messages_conversation_participants ON messages FOR SELECT TO authenticated USING (sender_id = clerk_user_id() OR recipient_id = clerk_user_id() OR clerk_is_admin() OR user_can_access_org_data(sender_id) OR user_can_access_org_data(recipient_id)) ; CREATE POLICY buddy_connection_members_invite ON buddy_connection_members FOR INSERT TO authenticated WITH CHECK (invited_by = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY property_interests_user_or_manager ON property_interests FOR SELECT TO authenticated USING (user_id = clerk_user_id() OR property_id IN (SELECT id FROM properties WHERE owner_id = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY storage_objects_insert_own ON storage.objects FOR INSERT TO authenticated WITH CHECK (clerk_user_id() IS NOT NULL AND (clerk_user_id() = split_part(name, '/', 1) OR current_clerk_org_id() = split_part(name, '/', 1) OR clerk_is_admin())) ; CREATE POLICY communities_member_write ON communities TO authenticated USING (creator_id = clerk_user_id() OR id IN (SELECT community_id FROM community_memberships WHERE user_id = clerk_user_id() AND status = 'active') OR clerk_is_admin()) WITH CHECK (creator_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY buddy_connection_members_participants ON buddy_connection_members FOR SELECT TO authenticated USING (invited_by = clerk_user_id() OR user_id = clerk_user_id() OR EXISTS (SELECT 1 FROM buddy_connections bc WHERE bc.id = buddy_connection_id AND bc.initiated_by = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY buddy_connection_members_leave ON buddy_connection_members FOR DELETE TO authenticated USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY storage_objects_org_shared ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'org-shared' AND current_clerk_org_id() = split_part(name, '/', 1)) ; CREATE POLICY property_interests_user_insert ON property_interests FOR INSERT TO authenticated WITH CHECK (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY community_posts_public_read ON community_posts FOR SELECT TO public USING (community_id IN (SELECT id FROM communities WHERE is_private = false) OR clerk_is_admin()) ; CREATE POLICY matches_user_participant ON matches TO authenticated USING (user1_id = clerk_user_id() OR user2_id = clerk_user_id() OR clerk_is_admin() OR user_can_access_org_data(user1_id) OR user_can_access_org_data(user2_id)) WITH CHECK (user1_id = clerk_user_id() OR user2_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY viewing_requests_user_or_owner ON viewing_requests TO authenticated USING (requester_id = clerk_user_id() OR property_id IN (SELECT id FROM properties WHERE owner_id = clerk_user_id()) OR clerk_is_admin()) WITH CHECK (requester_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY payment_transactions_user_only ON payment_transactions TO authenticated USING (payer_id = clerk_user_id() OR recipient_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (payer_id = clerk_user_id() OR recipient_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY messages_update_recipient ON messages FOR UPDATE TO authenticated USING (recipient_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY community_posts_member_write ON community_posts TO authenticated USING (author_id = clerk_user_id() OR community_id IN (SELECT community_id FROM community_memberships WHERE user_id = clerk_user_id() AND status = 'active') OR clerk_is_admin()) WITH CHECK (author_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY messages_insert_sender ON messages FOR INSERT TO authenticated WITH CHECK (sender_id = clerk_user_id()) ; CREATE POLICY buddy_connections_user_access ON buddy_connections TO authenticated USING (initiated_by = clerk_user_id() OR clerk_is_admin()) WITH CHECK (initiated_by = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY storage_objects_select_own ON storage.objects FOR SELECT TO authenticated USING (clerk_user_id() IS NOT NULL AND (clerk_user_id() = split_part(name, '/', 1) OR current_clerk_org_id() = split_part(name, '/', 1) OR clerk_is_admin())) ; CREATE POLICY conversations_participants_only ON conversations TO authenticated USING (participant_1_id = clerk_user_id() OR participant_2_id = clerk_user_id() OR clerk_is_admin() OR user_can_access_org_data(participant_1_id) OR user_can_access_org_data(participant_2_id)) WITH CHECK (participant_1_id = clerk_user_id() OR participant_2_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY communities_public_read ON communities FOR SELECT TO public USING (is_private = false OR clerk_is_admin()) ; CREATE POLICY storage_objects_delete_own ON storage.objects FOR DELETE TO authenticated USING (clerk_user_id() IS NOT NULL AND (clerk_user_id() = split_part(name, '/', 1) OR current_clerk_org_id() = split_part(name, '/', 1) OR clerk_is_admin())) ; CREATE POLICY community_memberships_user_and_admin ON community_memberships TO authenticated USING (user_id = clerk_user_id() OR community_id IN (SELECT id FROM communities WHERE creator_id = clerk_user_id()) OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY bookings_user_or_owner ON bookings TO authenticated USING (user_id = clerk_user_id() OR room_id IN (SELECT r.id FROM rooms r JOIN properties p ON r.property_id = p.id WHERE p.owner_id = clerk_user_id()) OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id() OR clerk_is_admin()) ; DO $$
DECLARE
