...
Guide for resolving common issues with pgsquash.
...
export PATH=$PATH:/path/to/pgsquash-go-app

# Or use absolute path
/path/to/pgsquash-engine/pgsquash analyze migrations/*.sql
```
...
# If psql works but pgsquash fails, syntax might be too new
```
...
**pgsquash automatically detects extensions**:
...
```bash
# pgsquash will warn about unsupported extensions
...
```bash
# List pgsquash containers
docker ps | grep pgsquash

# Stop all pgsquash containers
...
# Or let pgsquash use different ports (automatic)
```
...
1. **pgsquash version**: `pgsquash --version`
