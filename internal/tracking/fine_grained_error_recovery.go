...
func (r *FineGrainedErrorRecovery) RecoverFromError(err error, context map[string]interface{}) (*RecoveryResult, error) {
    ...
    return result, fmt.Errorf("all recovery strategies failed for error: %s", recoveryError.Message)
}

// RecoverFromMultipleErrors handles multiple errors with batch recovery
func (r *FineGrainedErrorRecovery) RecoverFromMultipleErrors(errors []error, context map[string]interface{}) ([]*RecoveryResult, error) {
    var results []*RecoveryResult
    var failureCount int

    r.logVerbose("Starting batch recovery for %d errors", len(errors))
    ...
    for i, err := range errors {
        ...
        errorContext["batch_total"] = len(errors)
    ...
    errorRate := float64(failureCount) / float64(len(errors))
    if errorRate > r.errorThreshold {
        return results, fmt.Errorf("error rate %.2f%% exceeded threshold %.2f%%", errorRate*100, r.errorThreshold*100)
    }

    r.logVerbose("Batch recovery completed: %d/%d succeeded", len(errors)-failureCount, len(errors))
    return results, nil
}
...
func (r *FineGrainedErrorRecovery) attemptRecovery(err *RecoveryError, strategy ErrorRecoveryStrategy) (*RecoveryResult, error) {
    ...
    default:
        recErr = fmt.Errorf("unknown recovery strategy: %s", strategy)
    }
...
func (r *FineGrainedErrorRecovery) analyzeError(err error, context map[string]interface{}) *RecoveryError {
...
func (r *FineGrainedErrorRecovery) tryRetryApproach(err *RecoveryError, attempt int, action *RecoveryAction) bool {
    ...
    case 1:
        ...
        action.Metadata = map[string]interface{}{"approach": "relaxed_constraints"}
    case 2:
        // Second retry: try with simplified SQL
        action.Description = "Retry with simplified SQL"
        action.Metadata = map[string]interface{}{"approach": "simplified_sql"}
    case 3:
        // Third retry: try with isolation
        action.Description = "Retry with object isolation"
        action.Metadata = map[string]interface{}{"approach": "isolation"}
    }
...
func (r *FineGrainedErrorRecovery) logVerbose(format string, args ...interface{}) {
    if r.enableVerbose {
        log.Printf("[ERROR_RECOVERY] "+format, args...)
    }
}
