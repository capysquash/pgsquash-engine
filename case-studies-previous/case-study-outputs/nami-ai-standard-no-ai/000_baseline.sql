...
$$; CREATE EXTENSION IF NOT EXISTS "uuid-ossp"; CREATE EXTENSION IF NOT EXISTS pg_trgm; CREATE EXTENSION IF NOT EXISTS pgcrypto; CREATE TABLE IF NOT EXISTS public.ai_providers (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), provider_id text UNIQUE NOT NULL, name text NOT NULL, provider_type text NOT NULL CHECK (provider_type IN ('openai', 'anthropic', 'azure', 'google', 'custom')), enabled boolean DEFAULT true, priority int DEFAULT 5 CHECK (priority >= 0 AND priority <= 10), model_name text, api_key_encrypted text, endpoint_url text, temperature numeric DEFAULT 0.7 CHECK (temperature >= 0 AND temperature <= 2), max_tokens int DEFAULT 4000, requests_per_minute int, tokens_per_minute int, last_health_check timestamp with time zone, health_status text DEFAULT 'unknown' CHECK (health_status IN ('healthy', 'unhealthy', 'unknown')), average_latency numeric, error_rate numeric DEFAULT 0, total_requests int DEFAULT 0, total_tokens int DEFAULT 0, total_cost_usd numeric DEFAULT 0.00, last_used timestamp with time zone, created_at timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now()); ALTER TABLE public.ai_providers ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.recovery_activities (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), name text NOT NULL, type text NOT NULL CHECK (type IN ('sensory', 'physical', 'mental', 'creative', 'restorative')), description text, duration_minutes int NOT NULL, energy_restoration_rate int NOT NULL CHECK (energy_restoration_rate >= 1 AND energy_restoration_rate <= 10), difficulty_level text NOT NULL CHECK (difficulty_level IN ('minimal', 'low', 'medium')), requirements text[] DEFAULT '{}', tags text[] DEFAULT '{}', icon text, is_default boolean DEFAULT false, is_active boolean DEFAULT true, created_at timestamp with time zone DEFAULT now()); ALTER TABLE public.recovery_activities ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.user_profiles (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), clerk_user_id text UNIQUE NOT NULL, email text, created_at timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now(), primary_support text CHECK (primary_support IN ('adhd', 'autism', 'dyslexia', 'executiveDysfunction', 'workingMemory', 'sensoryProcessing')), communication_style text DEFAULT 'balanced' CHECK (communication_style IN ('literal', 'balanced', 'warm')), preferred_response_length text DEFAULT 'standard' CHECK (preferred_response_length IN ('tldr', 'standard', 'detailed')), timing_preference text DEFAULT 'gentle-awareness' CHECK (timing_preference IN ('no-pressure', 'gentle-awareness', 'structured')), daily_cost_budget numeric DEFAULT 10.00, daily_token_limit int DEFAULT 5000, crisis_support_enabled boolean DEFAULT true, memory_sharing_enabled boolean DEFAULT false, analytics_enabled boolean DEFAULT true, success_patterns jsonb DEFAULT ('[]'::jsonb), stress_triggers jsonb DEFAULT ('[]'::jsonb), recovery_strategies jsonb DEFAULT ('[]'::jsonb), energy_patterns jsonb DEFAULT ('{}'::jsonb), timezone text DEFAULT 'UTC', language text DEFAULT 'en', accessibility_preferences jsonb DEFAULT ('{}'::jsonb), energy_level text DEFAULT 'medium' CHECK (energy_level IN ('low', 'medium', 'high')), ui_state text DEFAULT 'explore' CHECK (ui_state IN ('calm', 'focus', 'explore', 'crisis', 'flow', 'social', 'planning', 'recovery')), task_breakdown_enabled boolean DEFAULT true, progress_celebration_enabled boolean DEFAULT true, current_stress_level int DEFAULT 3 CHECK (current_stress_level >= 1 AND current_stress_level <= 10), current_social_battery int DEFAULT 8 CHECK (current_social_battery >= 0 AND current_social_battery <= 10), burnout_risk_score int DEFAULT 0 CHECK (burnout_risk_score >= 0 AND burnout_risk_score <= 100), crisis_mode_active boolean DEFAULT false, last_crisis_event timestamp with time zone, onboarding_completed boolean DEFAULT false, onboarding_responses jsonb DEFAULT ('{}'::jsonb), first_name text, last_name text, current_org_id text, current_org_role text, current_org_name text); ALTER TABLE public.user_profiles ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.usage_analytics (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), date date NOT NULL, primary_support text, energy_level text, ui_state text, total_conversations int DEFAULT 0, total_tokens int DEFAULT 0, total_cost_usd numeric DEFAULT 0.00, tools_used jsonb DEFAULT ('[]'::jsonb), average_response_time numeric, error_count int DEFAULT 0, successful_interactions int DEFAULT 0, memory_cards_created int DEFAULT 0, crisis_interventions int DEFAULT 0, helpful_responses int DEFAULT 0, created_at timestamp with time zone DEFAULT now(), UNIQUE (date, primary_support, energy_level, ui_state)); ALTER TABLE public.usage_analytics ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.system_monitoring (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), "timestamp" timestamp with time zone DEFAULT now(), cpu_usage_percent numeric, memory_usage_percent numeric, disk_usage_percent numeric, database_connections_active int, database_connections_max int, avg_response_time_ms numeric, error_rate_percent numeric, requests_per_minute int, active_users int, active_crisis_sessions int DEFAULT 0, high_risk_users int DEFAULT 0, crisis_interventions_today int DEFAULT 0, provider_health jsonb DEFAULT ('{}'::jsonb), custom_metrics jsonb DEFAULT ('{}'::jsonb)); ALTER TABLE public.system_monitoring ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.system_analytics (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), date date NOT NULL, period_type text NOT NULL CHECK (period_type IN ('hour', 'day', 'week', 'month')), total_conversations int DEFAULT 0, active_users int DEFAULT 0, new_users int DEFAULT 0, returning_users int DEFAULT 0, crisis_interventions int DEFAULT 0, crisis_resolutions int DEFAULT 0, auto_detection_rate numeric DEFAULT 0, intervention_success_rate numeric DEFAULT 0, avg_response_time_ms numeric DEFAULT 0, uptime_percentage numeric DEFAULT 100, error_count int DEFAULT 0, provider_usage jsonb DEFAULT ('{}'::jsonb), provider_costs jsonb DEFAULT ('{}'::jsonb), recovery_sessions_started int DEFAULT 0, recovery_sessions_completed int DEFAULT 0, avg_recovery_effectiveness numeric DEFAULT 0, created_at timestamp with time zone DEFAULT now(), UNIQUE (date, period_type)); ALTER TABLE public.system_analytics ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.admin_action_logs (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), admin_user_id text NOT NULL, admin_email text, action_type text NOT NULL CHECK (action_type IN ('user_activate', 'user_deactivate', 'user_exit_crisis', 'provider_create', 'provider_update', 'provider_delete', 'provider_test', 'settings_update', 'system_restart', 'data_export', 'crisis_intervention_manual', 'emergency_broadcast')), resource_type text NOT NULL CHECK (resource_type IN ('user', 'provider', 'setting', 'system')), resource_id text, action_details jsonb DEFAULT ('{}'::jsonb), previous_state jsonb, new_state jsonb, success boolean NOT NULL, error_message text, ip_address inet, user_agent text, "timestamp" timestamp with time zone DEFAULT now()); ALTER TABLE public.admin_action_logs ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.system_settings (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), category text NOT NULL CHECK (category IN ('rate_limit', 'security', 'crisis', 'monitoring', 'general')), setting_key text NOT NULL, setting_value jsonb NOT NULL, description text, is_public boolean DEFAULT false, requires_restart boolean DEFAULT false, version int DEFAULT 1, previous_value jsonb, created_by text, created_at timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now(), UNIQUE (category, setting_key)); ALTER TABLE public.system_settings ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.recovery_sessions (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid REFERENCES public.user_profiles (id) ON DELETE CASCADE, activity_id uuid REFERENCES public.recovery_activities (id), start_time timestamp with time zone DEFAULT now(), end_time timestamp with time zone, planned_duration_minutes int NOT NULL, actual_duration_minutes int, energy_before int CHECK (energy_before >= 1 AND energy_before <= 10), stress_before int CHECK (stress_before >= 1 AND stress_before <= 10), energy_after int CHECK (energy_after >= 1 AND energy_after <= 10), stress_after int CHECK (stress_after >= 1 AND stress_after <= 10), quality text CHECK (quality IN ('poor', 'good', 'excellent')), completion_rate int DEFAULT 100 CHECK (completion_rate >= 0 AND completion_rate <= 100), interruptions int DEFAULT 0, notes text, environmental_settings jsonb DEFAULT ('{}'::jsonb), was_scheduled boolean DEFAULT false, was_emergency boolean DEFAULT false, created_at timestamp with time zone DEFAULT now()); ALTER TABLE public.recovery_sessions ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.behavioral_patterns (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid REFERENCES public.user_profiles (id) ON DELETE CASCADE, interaction_type text NOT NULL CHECK (interaction_type IN ('click', 'input', 'scroll', 'focus', 'blur', 'keypress', 'error')), "timestamp" timestamp with time zone DEFAULT now(), duration_ms int, intensity numeric, metadata jsonb DEFAULT ('{}'::jsonb), session_id text NOT NULL, created_at timestamp with time zone DEFAULT now()); ALTER TABLE public.behavioral_patterns ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.energy_patterns (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid REFERENCES public.user_profiles (id) ON DELETE CASCADE, energy_level int NOT NULL CHECK (energy_level >= 1 AND energy_level <= 10), stress_level int NOT NULL CHECK (stress_level >= 1 AND stress_level <= 10), cognitive_load int NOT NULL CHECK (cognitive_load >= 1 AND cognitive_load <= 10), social_battery int NOT NULL CHECK (social_battery >= 1 AND social_battery <= 10), activity_type text NOT NULL, ui_state text, time_of_day int CHECK (time_of_day >= 0 AND time_of_day <= 23), day_of_week int CHECK (day_of_week >= 0 AND day_of_week <= 6), sleep_quality int CHECK (sleep_quality >= 1 AND sleep_quality <= 10), physical_comfort int CHECK (physical_comfort >= 1 AND physical_comfort <= 10), "timestamp" timestamp with time zone DEFAULT now()); ALTER TABLE public.energy_patterns ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.task_templates (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid REFERENCES public.user_profiles (id) ON DELETE CASCADE, name text NOT NULL, title_template text NOT NULL, description_template text, category text NOT NULL, default_priority text DEFAULT 'medium', default_energy_cost int DEFAULT 3, default_estimated_time int DEFAULT 30, default_breakdown_level int DEFAULT 3, default_hyperfocus_warning boolean DEFAULT false, default_transition_support boolean DEFAULT false, default_sensory_considerations text[] DEFAULT '{}', times_used int DEFAULT 0, last_used timestamp with time zone, created_at timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now(), UNIQUE (user_id, name)); ALTER TABLE public.task_templates ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.crisis_support_logs (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid REFERENCES public.user_profiles (id) ON DELETE CASCADE, crisis_type text CHECK (crisis_type IN ('overwhelm', 'anxiety', 'meltdown', 'shutdown', 'general')), severity text CHECK (severity IN ('mild', 'moderate', 'severe')), trigger_context text, strategies_provided jsonb DEFAULT ('[]'::jsonb), resources_offered jsonb DEFAULT ('[]'::jsonb), user_feedback_immediate text, follow_up_needed boolean DEFAULT false, professional_referral_suggested boolean DEFAULT false, emergency_resources_provided boolean DEFAULT false, crisis_started timestamp with time zone DEFAULT now(), crisis_resolved timestamp with time zone, follow_up_date timestamp with time zone, anonymized boolean DEFAULT false); ALTER TABLE public.crisis_support_logs ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.recovery_plans (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid REFERENCES public.user_profiles (id) ON DELETE CASCADE, name text NOT NULL, description text, target_recovery_time_minutes int NOT NULL, activity_ids uuid[] DEFAULT '{}', break_intervals_minutes int DEFAULT 5, adaptive_difficulty boolean DEFAULT true, environmental_needs text[] DEFAULT '{}', emergency_protocols text[] DEFAULT '{}', times_used int DEFAULT 0, effectiveness_rating numeric DEFAULT 0, last_used timestamp with time zone, is_active boolean DEFAULT true, is_default boolean DEFAULT false, created_at timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now()); ALTER TABLE public.recovery_plans ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.user_activity_snapshots (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid REFERENCES public.user_profiles (id) ON DELETE CASCADE, snapshot_date timestamp with time zone DEFAULT now(), total_conversations int DEFAULT 0, conversations_last_7_days int DEFAULT 0, conversations_last_30_days int DEFAULT 0, last_active timestamp with time zone, current_energy_level int, current_stress_level int, in_crisis_mode boolean DEFAULT false, crisis_events_count int DEFAULT 0, recovery_sessions_count int DEFAULT 0, avg_recovery_effectiveness numeric DEFAULT 0, memory_cards_count int DEFAULT 0, memory_cards_accessed_last_week int DEFAULT 0, is_active boolean DEFAULT true, requires_admin_attention boolean DEFAULT false, attention_reason text, data_retention_expires timestamp with time zone); ALTER TABLE public.user_activity_snapshots ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.task_categories (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid REFERENCES public.user_profiles (id) ON DELETE CASCADE, name text NOT NULL, color text DEFAULT '#3B82F6', icon text DEFAULT 'folder', default_energy_cost int DEFAULT 3, default_time_estimate int DEFAULT 30, auto_breakdown boolean DEFAULT false, created_at timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now(), UNIQUE (user_id, name)); ALTER TABLE public.task_categories ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.memory_cards (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid REFERENCES public.user_profiles (id) ON DELETE CASCADE, trigger_phrase text NOT NULL, content text NOT NULL, context_info text, category text NOT NULL CHECK (category IN ('personal', 'work', 'health', 'projects', 'communications', 'learning', 'coping', 'achievements')), importance text DEFAULT 'medium' CHECK (importance IN ('low', 'medium', 'high', 'critical')), cognitive_type text DEFAULT 'fact' CHECK (cognitive_type IN ('fact', 'process', 'template', 'reminder', 'accomplishment', 'strategy')), energy_level_created text CHECK (energy_level_created IN ('low', 'medium', 'high', 'crisis')), ui_state_created text, accessibility_notes text, created_at timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now(), last_accessed timestamp with time zone DEFAULT now(), access_count int DEFAULT 0, relevance_score numeric DEFAULT 1.0, surfacing_keywords text[] DEFAULT '{}', surfacing_contexts text[] DEFAULT '{}', surfacing_energy_states text[] DEFAULT '{}', surfacing_ui_states text[] DEFAULT '{}', daily_reminder boolean DEFAULT false, specific_times time[], day_of_week int[], related_card_ids uuid[] DEFAULT '{}', tags text[] DEFAULT '{}', search_vector tsvector); ALTER TABLE public.memory_cards ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.conversations (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid REFERENCES public.user_profiles (id) ON DELETE CASCADE, conversation_id text NOT NULL, user_message text NOT NULL, ai_response text NOT NULL, response_metadata jsonb DEFAULT ('{}'::jsonb), ui_state text, energy_level text, cognitive_load text, "timestamp" timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now(), user_feedback int CHECK (user_feedback IN (-1, 0, 1)), was_helpful boolean, follow_up_needed boolean DEFAULT false, extracted_facts jsonb DEFAULT ('[]'::jsonb), extracted_learnings jsonb DEFAULT ('[]'::jsonb), extracted_preferences jsonb DEFAULT ('[]'::jsonb), extracted_strategies jsonb DEFAULT ('[]'::jsonb), archived boolean DEFAULT false, archive_date timestamp with time zone); ALTER TABLE public.conversations ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.social_interactions (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid REFERENCES public.user_profiles (id) ON DELETE CASCADE, interaction_type text NOT NULL CHECK (interaction_type IN ('meeting', 'chat', 'call', 'email', 'social_media', 'group', 'presentation', 'networking', 'other')), duration_minutes int, energy_cost int CHECK (energy_cost >= 1 AND energy_cost <= 10), energy_gained int CHECK (energy_gained >= 0 AND energy_gained <= 10), context text, people_count int DEFAULT 1, was_planned boolean DEFAULT false, started_at timestamp with time zone DEFAULT now(), ended_at timestamp with time zone, created_at timestamp with time zone DEFAULT now()); ALTER TABLE public.social_interactions ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.planning_sessions (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid REFERENCES public.user_profiles (id) ON DELETE CASCADE, focus_area text NOT NULL, start_time timestamp with time zone NOT NULL DEFAULT now(), end_time timestamp with time zone, tasks_planned int DEFAULT 0, tasks_completed int DEFAULT 0, energy_used int DEFAULT 0, quality text CHECK (quality IN ('poor', 'good', 'excellent')), notes text, achievements jsonb DEFAULT ('[]'::jsonb), lessons_learned jsonb DEFAULT ('[]'::jsonb), created_at timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now()); ALTER TABLE public.planning_sessions ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.tasks (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid REFERENCES public.user_profiles (id) ON DELETE CASCADE, title text NOT NULL, description text, priority text NOT NULL CHECK (priority IN ('low', 'medium', 'high', 'urgent')), energy_cost int NOT NULL CHECK (energy_cost >= 1 AND energy_cost <= 10), estimated_time int NOT NULL, actual_time int, completed boolean DEFAULT false, category text NOT NULL, deadline timestamp with time zone, breakdown_level int DEFAULT 3 CHECK (breakdown_level >= 0 AND breakdown_level <= 8), cognitive_load int NOT NULL CHECK (cognitive_load >= 1 AND cognitive_load <= 10), sequencing boolean DEFAULT true, timing_support boolean DEFAULT true, hyperfocus_warning boolean DEFAULT false, transition_support boolean DEFAULT false, sensory_considerations text[] DEFAULT '{}', dopamine_boosts text[] DEFAULT '{}', created_at timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now(), completed_at timestamp with time zone); ALTER TABLE public.tasks ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.planning_preferences (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid REFERENCES public.user_profiles (id) ON DELETE CASCADE UNIQUE, max_tasks_per_day int DEFAULT 8, preferred_planning_time text DEFAULT 'morning', use_time_blocking boolean DEFAULT false, enable_energy_tracking boolean DEFAULT true, executive_function_level int DEFAULT 5 CHECK (executive_function_level >= 1 AND executive_function_level <= 10), auto_breakdown_threshold int DEFAULT 60, hyperfocus_mode boolean DEFAULT false, transition_time int DEFAULT 5, cognitive_load_warnings boolean DEFAULT true, pomodoro_enabled boolean DEFAULT false, pomodoro_work_duration int DEFAULT 25, pomodoro_break_duration int DEFAULT 5, deadline_warnings boolean DEFAULT true, energy_budget_alerts boolean DEFAULT true, daily_planning_reminder boolean DEFAULT true, daily_planning_time time DEFAULT '09:00', created_at timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now()); ALTER TABLE public.planning_preferences ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.crisis_events (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid REFERENCES public.user_profiles (id) ON DELETE CASCADE, severity_level text NOT NULL CHECK (severity_level IN ('mild', 'moderate', 'severe', 'critical')), confidence_score numeric NOT NULL CHECK (confidence_score >= 0 AND confidence_score <= 100), escalation_risk numeric NOT NULL CHECK (escalation_risk >= 0 AND escalation_risk <= 100), primary_triggers text[] DEFAULT '{}', detected_indicators jsonb DEFAULT ('{}'::jsonb), recommended_interventions text[] DEFAULT '{}', time_to_intervention_minutes int, intervention_taken text, intervention_successful boolean, detection_timestamp timestamp with time zone DEFAULT now(), intervention_timestamp timestamp with time zone, resolution_timestamp timestamp with time zone, user_feedback text, follow_up_needed boolean DEFAULT false, follow_up_completed boolean DEFAULT false, detection_method text DEFAULT 'automatic', false_positive boolean DEFAULT false, created_at timestamp with time zone DEFAULT now()); ALTER TABLE public.crisis_events ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.crisis_baselines (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid REFERENCES public.user_profiles (id) ON DELETE CASCADE UNIQUE, avg_clicks_per_minute numeric DEFAULT 5.0, avg_response_length int DEFAULT 25, avg_error_rate numeric DEFAULT 0.05, avg_task_switches int DEFAULT 2, typical_response_time_ms int DEFAULT 3000, avg_energy_level numeric DEFAULT 7.0, avg_stress_level numeric DEFAULT 3.0, avg_cognitive_load numeric DEFAULT 4.0, avg_social_battery numeric DEFAULT 8.0, avg_session_duration_minutes int DEFAULT 45, preferred_working_hours int[] DEFAULT '{9,10,11,12,13,14,15,16,17}', sample_size int DEFAULT 0, last_calculated timestamp with time zone DEFAULT now(), confidence_score numeric DEFAULT 0.5, created_at timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now()); ALTER TABLE public.crisis_baselines ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.recovery_preferences (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid REFERENCES public.user_profiles (id) ON DELETE CASCADE UNIQUE, preferred_activity_types text[] DEFAULT '{}', max_session_duration_minutes int DEFAULT 60, adaptive_scheduling boolean DEFAULT true, gentle_reminders boolean DEFAULT true, progress_tracking boolean DEFAULT true, sensory_regulation boolean DEFAULT true, environmental_controls jsonb DEFAULT ('{
    ...
    }'::jsonb), created_at timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now()); ALTER TABLE public.recovery_preferences ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.social_battery (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), user_id uuid REFERENCES public.user_profiles (id) ON DELETE CASCADE, current_level int NOT NULL CHECK (current_level >= 0 AND current_level <= 100), max_capacity int NOT NULL DEFAULT 100 CHECK (max_capacity >= 1 AND max_capacity <= 100), drain_rate numeric DEFAULT 1.0, recharge_rate numeric DEFAULT 2.0, last_interaction_id uuid REFERENCES public.social_interactions (id), last_recovery_session_id uuid REFERENCES public.recovery_sessions (id), "timestamp" timestamp with time zone DEFAULT now(), UNIQUE (user_id, "timestamp")); ALTER TABLE public.social_battery ENABLE ROW LEVEL SECURITY; CREATE TABLE IF NOT EXISTS public.subtasks (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), task_id uuid REFERENCES public.tasks (id) ON DELETE CASCADE, title text NOT NULL, completed boolean DEFAULT false, estimated_time int NOT NULL, order_position int NOT NULL, created_at timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now(), completed_at timestamp with time zone); ALTER TABLE public.subtasks ENABLE ROW LEVEL SECURITY; CREATE OR REPLACE FUNCTION current_clerk_org_role() RETURNS text LANGUAGE sql VOLATILE AS $$
  SELECT (auth.jwt()->'o'->>'role')::TEXT;
$$ SECURITY DEFINER; CREATE OR REPLACE FUNCTION public.get_user_profile_data(clerk_id text) RETURNS TABLE (id uuid, communication_style text, preferred_response_length text, timing_preference text, primary_support text, daily_cost_budget numeric, daily_token_limit int, accessibility_preferences jsonb, energy_patterns jsonb) LANGUAGE plpgsql SECURITY DEFINER VOLATILE AS $$
BEGIN
...
$$ SECURITY DEFINER; CREATE OR REPLACE FUNCTION public.cleanup_expired_memory_cards() RETURNS int LANGUAGE plpgsql SECURITY DEFINER VOLATILE AS $$
declare
...
$$ SECURITY DEFINER; CREATE OR REPLACE FUNCTION public.generate_daily_analytics(target_date date = current_date) RETURNS void LANGUAGE plpgsql SECURITY DEFINER VOLATILE AS $$
declare
...
BEGIN
    ...
    LIMIT limit_results;
END;
$$; CREATE OR REPLACE FUNCTION validate_jwt_version() RETURNS boolean LANGUAGE sql VOLATILE AS $$
  SELECT COALESCE((auth.jwt()->'v')::INTEGER >= 2, false);
$$ SECURITY DEFINER; CREATE OR REPLACE FUNCTION public.get_user_preferences(clerk_id text) RETURNS pg_catalog.json LANGUAGE plpgsql SECURITY DEFINER VOLATILE AS $$
DECLARE
    user_prefs json;
BEGIN
...
$$; CREATE OR REPLACE FUNCTION get_planning_analytics(user_uuid uuid, days_back int = 30) RETURNS jsonb LANGUAGE plpgsql VOLATILE AS $$
declare
    result jsonb;
    total_tasks integer;
...
$$ SECURITY DEFINER; CREATE OR REPLACE FUNCTION public.update_user_baselines(user_uuid uuid) RETURNS void LANGUAGE plpgsql SECURITY DEFINER VOLATILE AS $$
declare
...
BEGIN
  ...
  SELECT 
    ...
    END as status;
END $$ ; CREATE OR REPLACE FUNCTION public.set_session_user(clerk_user_id text) RETURNS void LANGUAGE plpgsql SECURITY DEFINER STABLE AS $$
BEGIN
    ...
    END IF;
END;
$$; CREATE TRIGGER update_memory_relevance_trigger BEFORE UPDATE ON public.memory_cards FOR EACH ROW EXECUTE FUNCTION public.update_memory_card_relevance(); CREATE TRIGGER update_user_profiles_updated_at BEFORE UPDATE ON public.user_profiles FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_task_templates_updated_at BEFORE UPDATE ON public.task_templates FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_planning_sessions_updated_at BEFORE UPDATE ON public.planning_sessions FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_memory_search_vector_trigger BEFORE INSERT OR UPDATE ON public.memory_cards FOR EACH ROW EXECUTE FUNCTION public.update_memory_card_search_vector(); CREATE TRIGGER update_subtasks_updated_at BEFORE UPDATE ON public.subtasks FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_memory_cards_updated_at BEFORE UPDATE ON public.memory_cards FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER auto_complete_task_on_subtasks AFTER UPDATE ON public.subtasks FOR EACH ROW EXECUTE FUNCTION check_task_completion(); CREATE TRIGGER update_conversations_updated_at BEFORE UPDATE ON public.conversations FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_memory_card_timestamp_trigger BEFORE UPDATE ON public.memory_cards FOR EACH ROW WHEN (old.access_count IS DISTINCT FROM new.access_count) EXECUTE FUNCTION public.update_memory_card_timestamp(); CREATE TRIGGER update_task_categories_updated_at BEFORE UPDATE ON public.task_categories FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_tasks_updated_at BEFORE UPDATE ON public.tasks FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_planning_preferences_updated_at BEFORE UPDATE ON public.planning_preferences FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE INDEX IF NOT EXISTS idx_crisis_events_unresolved ON public.crisis_events USING btree (resolution_timestamp) WHERE resolution_timestamp IS NULL; CREATE INDEX IF NOT EXISTS idx_planning_sessions_active ON public.planning_sessions USING btree (user_id, end_time) WHERE end_time IS NULL; CREATE INDEX IF NOT EXISTS idx_crisis_logs_timestamp ON public.crisis_support_logs USING btree (crisis_started); CREATE INDEX IF NOT EXISTS idx_recovery_activities_type ON public.recovery_activities USING btree (type); CREATE INDEX IF NOT EXISTS idx_tasks_priority ON public.tasks USING btree (priority); CREATE INDEX IF NOT EXISTS idx_task_categories_user_id ON public.task_categories USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_subtasks_task_id ON public.subtasks USING btree (task_id); CREATE INDEX IF NOT EXISTS idx_admin_action_logs_action_type ON public.admin_action_logs USING btree (action_type); CREATE INDEX IF NOT EXISTS idx_conversations_updated_at ON public.conversations USING btree (updated_at DESC); CREATE INDEX IF NOT EXISTS idx_social_interactions_started_at ON public.social_interactions USING btree (started_at); CREATE INDEX IF NOT EXISTS idx_behavioral_patterns_user_id ON public.behavioral_patterns USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_crisis_logs_user_id ON public.crisis_support_logs USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_memory_cards_user_energy ON public.memory_cards USING gin (surfacing_energy_states); CREATE INDEX IF NOT EXISTS idx_user_activity_snapshots_user_id ON public.user_activity_snapshots USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_task_templates_user_id ON public.task_templates USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_user_profiles_crisis_mode ON public.user_profiles USING btree (crisis_mode_active) WHERE crisis_mode_active = true; CREATE INDEX IF NOT EXISTS idx_user_activity_snapshots_attention ON public.user_activity_snapshots USING btree (requires_admin_attention) WHERE requires_admin_attention = true; CREATE INDEX IF NOT EXISTS idx_system_settings_key ON public.system_settings USING btree (setting_key); CREATE INDEX IF NOT EXISTS idx_crisis_events_user_id ON public.crisis_events USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_user_profiles_onboarding_completed ON public.user_profiles USING btree (onboarding_completed); CREATE INDEX IF NOT EXISTS idx_user_profiles_stress_level ON public.user_profiles USING btree (current_stress_level); CREATE INDEX IF NOT EXISTS idx_recovery_plans_active ON public.recovery_plans USING btree (is_active); CREATE INDEX IF NOT EXISTS idx_social_battery_timestamp ON public.social_battery USING btree ("timestamp"); CREATE INDEX IF NOT EXISTS idx_memory_cards_updated_at ON public.memory_cards USING btree (updated_at DESC); CREATE INDEX IF NOT EXISTS idx_system_analytics_date ON public.system_analytics USING btree (date); CREATE INDEX IF NOT EXISTS idx_tasks_user_id ON public.tasks USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_user_profiles_ui_state ON public.user_profiles USING btree (ui_state); CREATE INDEX IF NOT EXISTS idx_memory_cards_search_vector ON public.memory_cards USING gin (search_vector); CREATE INDEX IF NOT EXISTS idx_user_profiles_burnout_risk ON public.user_profiles USING btree (burnout_risk_score) WHERE burnout_risk_score > 50; CREATE INDEX IF NOT EXISTS idx_admin_action_logs_admin ON public.admin_action_logs USING btree (admin_user_id); CREATE INDEX IF NOT EXISTS idx_recovery_sessions_quality ON public.recovery_sessions USING btree (quality); CREATE INDEX IF NOT EXISTS idx_system_settings_public ON public.system_settings USING btree (is_public) WHERE is_public = true; CREATE INDEX IF NOT EXISTS idx_user_activity_snapshots_date ON public.user_activity_snapshots USING btree (snapshot_date); CREATE INDEX IF NOT EXISTS idx_planning_sessions_start_time ON public.planning_sessions USING btree (start_time); CREATE INDEX IF NOT EXISTS idx_subtasks_completed ON public.subtasks USING btree (completed); CREATE INDEX IF NOT EXISTS idx_behavioral_patterns_timestamp ON public.behavioral_patterns USING btree ("timestamp"); CREATE INDEX IF NOT EXISTS idx_memory_cards_user_updated ON public.memory_cards USING btree (user_id, updated_at DESC); CREATE INDEX IF NOT EXISTS idx_memory_cards_user_contexts ON public.memory_cards USING gin (surfacing_contexts); CREATE INDEX IF NOT EXISTS idx_recovery_plans_user_id ON public.recovery_plans USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_system_analytics_period ON public.system_analytics USING btree (period_type); CREATE INDEX IF NOT EXISTS idx_recovery_activities_difficulty ON public.recovery_activities USING btree (difficulty_level); CREATE INDEX IF NOT EXISTS idx_energy_patterns_activity ON public.energy_patterns USING btree (activity_type); CREATE INDEX IF NOT EXISTS idx_recovery_activities_active ON public.recovery_activities USING btree (is_active); CREATE INDEX IF NOT EXISTS idx_user_profiles_energy_level ON public.user_profiles USING btree (energy_level); CREATE INDEX IF NOT EXISTS idx_energy_patterns_timestamp ON public.energy_patterns USING btree ("timestamp"); CREATE INDEX IF NOT EXISTS idx_subtasks_order ON public.subtasks USING btree (task_id, order_position); CREATE INDEX IF NOT EXISTS idx_usage_analytics_date ON public.usage_analytics USING btree (date); CREATE INDEX IF NOT EXISTS idx_crisis_events_detection_time ON public.crisis_events USING btree (detection_timestamp); CREATE INDEX IF NOT EXISTS idx_tasks_created_at ON public.tasks USING btree (created_at); CREATE INDEX IF NOT EXISTS idx_tasks_category ON public.tasks USING btree (category); CREATE INDEX IF NOT EXISTS idx_social_interactions_interaction_type ON public.social_interactions USING btree (interaction_type); CREATE INDEX IF NOT EXISTS idx_conversations_timestamp ON public.conversations USING btree ("timestamp"); CREATE INDEX IF NOT EXISTS idx_recovery_sessions_start_time ON public.recovery_sessions USING btree (start_time); CREATE INDEX IF NOT EXISTS idx_user_profiles_clerk_id ON public.user_profiles USING btree (clerk_user_id); CREATE INDEX IF NOT EXISTS idx_memory_cards_user_id ON public.memory_cards USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_behavioral_patterns_type ON public.behavioral_patterns USING btree (interaction_type); CREATE INDEX IF NOT EXISTS idx_user_activity_snapshots_active ON public.user_activity_snapshots USING btree (is_active); CREATE INDEX IF NOT EXISTS idx_system_monitoring_recent ON public.system_monitoring USING btree ("timestamp" DESC); CREATE INDEX IF NOT EXISTS idx_tasks_completed ON public.tasks USING btree (completed); CREATE INDEX IF NOT EXISTS idx_crisis_baselines_user_id ON public.crisis_baselines USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_planning_sessions_user_id ON public.planning_sessions USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_recovery_sessions_user_id ON public.recovery_sessions USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_tasks_deadline ON public.tasks USING btree (deadline); CREATE INDEX IF NOT EXISTS idx_memory_cards_category ON public.memory_cards USING btree (category); CREATE INDEX IF NOT EXISTS idx_memory_cards_importance ON public.memory_cards USING btree (importance); CREATE INDEX IF NOT EXISTS idx_memory_cards_keywords ON public.memory_cards USING gin (surfacing_keywords); CREATE INDEX IF NOT EXISTS idx_system_monitoring_timestamp ON public.system_monitoring USING btree ("timestamp"); CREATE INDEX IF NOT EXISTS idx_behavioral_patterns_session ON public.behavioral_patterns USING btree (session_id); CREATE INDEX IF NOT EXISTS idx_energy_patterns_user_id ON public.energy_patterns USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_social_interactions_user_id ON public.social_interactions USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_system_settings_category ON public.system_settings USING btree (category); CREATE INDEX IF NOT EXISTS idx_recovery_sessions_activity_id ON public.recovery_sessions USING btree (activity_id); CREATE INDEX IF NOT EXISTS idx_conversations_user_id ON public.conversations USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_admin_action_logs_timestamp ON public.admin_action_logs USING btree ("timestamp"); CREATE INDEX IF NOT EXISTS idx_crisis_events_severity ON public.crisis_events USING btree (severity_level); CREATE INDEX IF NOT EXISTS idx_user_profiles_primary_support ON public.user_profiles USING btree (primary_support); CREATE INDEX IF NOT EXISTS idx_memory_cards_tags ON public.memory_cards USING gin (tags); CREATE INDEX IF NOT EXISTS idx_recovery_preferences_user_id ON public.recovery_preferences USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_memory_cards_user_ui_states ON public.memory_cards USING gin (surfacing_ui_states); CREATE INDEX IF NOT EXISTS idx_admin_action_logs_resource ON public.admin_action_logs USING btree (resource_type, resource_id); CREATE INDEX IF NOT EXISTS idx_social_battery_user_id ON public.social_battery USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_conversations_conversation_id ON public.conversations USING btree (conversation_id); CREATE POLICY admin_analytics_jwt_v2 ON public.system_analytics TO authenticated USING ((current_clerk_org_role() IN ('admin', 'owner') OR (auth.jwt() ->> 'role') = 'service_role') AND validate_jwt_version()) ; CREATE POLICY behavioral_patterns_jwt_v2 ON public.behavioral_patterns TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY social_battery_jwt_v2 ON public.social_battery TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY recovery_activities_admin_update ON public.recovery_activities FOR UPDATE TO authenticated USING ((auth.jwt() ->> 'role') = 'service_role' AND validate_jwt_version()) WITH CHECK ((auth.jwt() ->> 'role') = 'service_role' AND validate_jwt_version()) ; CREATE POLICY planning_preferences_user_access_jwt_v2 ON public.planning_preferences TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY recovery_activities_admin_delete ON public.recovery_activities FOR DELETE TO authenticated USING ((auth.jwt() ->> 'role') = 'service_role' AND validate_jwt_version()) ; CREATE POLICY crisis_logs_jwt_v2 ON public.crisis_support_logs TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY task_templates_user_access_jwt_v2 ON public.task_templates TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY memory_cards_jwt_v2 ON public.memory_cards TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY admin_monitoring_data_jwt_v2 ON public.system_monitoring TO authenticated USING ((current_clerk_org_role() IN ('admin', 'owner') OR (auth.jwt() ->> 'role') = 'service_role') AND validate_jwt_version()) ; CREATE POLICY energy_patterns_jwt_v2 ON public.energy_patterns TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY crisis_baselines_jwt_v2 ON public.crisis_baselines TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY crisis_events_jwt_v2 ON public.crisis_events TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY tasks_user_access_jwt_v2 ON public.tasks TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY public_settings_readable_jwt_v2 ON public.system_settings FOR SELECT TO authenticated USING (is_public = true AND validate_jwt_version()) ; CREATE POLICY planning_sessions_user_access_jwt_v2 ON public.planning_sessions TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY ai_providers_service_only ON public.ai_providers TO authenticated USING ((auth.jwt() ->> 'role') = 'service_role' AND validate_jwt_version()) ; CREATE POLICY admin_user_snapshots_jwt_v2 ON public.user_activity_snapshots TO authenticated USING ((current_clerk_org_role() IN ('admin', 'owner') OR (auth.jwt() ->> 'role') = 'service_role') AND validate_jwt_version()) ; CREATE POLICY conversations_jwt_v2 ON public.conversations TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY user_profiles_jwt_v2 ON public.user_profiles TO authenticated USING (clerk_user_id = current_clerk_user_id() AND validate_jwt_version()) ; CREATE POLICY admin_action_logs_jwt_v2 ON public.admin_action_logs TO authenticated USING ((current_clerk_org_role() IN ('admin', 'owner') OR (auth.jwt() ->> 'role') = 'service_role') AND validate_jwt_version()) ; CREATE POLICY usage_analytics_service_only ON public.usage_analytics TO authenticated USING ((auth.jwt() ->> 'role') = 'service_role' AND validate_jwt_version()) ; CREATE POLICY subtasks_user_access_jwt_v2 ON public.subtasks TO authenticated USING (task_id IN (SELECT t.id FROM public.tasks t JOIN public.user_profiles up ON t.user_id = up.id WHERE up.clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY recovery_preferences_jwt_v2 ON public.recovery_preferences TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY recovery_sessions_jwt_v2 ON public.recovery_sessions TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY recovery_activities_admin_modify ON public.recovery_activities FOR INSERT TO authenticated WITH CHECK ((auth.jwt() ->> 'role') = 'service_role' AND validate_jwt_version()) ; CREATE POLICY task_categories_user_access_jwt_v2 ON public.task_categories TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY social_interactions_jwt_v2 ON public.social_interactions TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY recovery_activities_read_all ON public.recovery_activities FOR SELECT TO authenticated USING (validate_jwt_version()) ; CREATE POLICY recovery_plans_jwt_v2 ON public.recovery_plans TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY admin_system_settings_jwt_v2 ON public.system_settings TO authenticated USING ((current_clerk_org_role() IN ('admin', 'owner') OR (auth.jwt() ->> 'role') = 'service_role') AND validate_jwt_version()) ; UPDATE public.memory_cards SET search_vector = (setweight(to_tsvector('english', COALESCE(trigger_phrase, '')), 'A') || setweight(to_tsvector('english', COALESCE(content, '')), 'B')) || setweight(to_tsvector('english', COALESCE(array_to_string(tags, ' '), '')), 'C') WHERE search_vector IS NULL; INSERT INTO public.recovery_activities (name, type, description, duration_minutes, energy_restoration_rate, difficulty_level, requirements, tags, icon, is_default) VALUES ('5-Minute Breathing', 'mental', 'Simple breathing exercises to reduce stress and restore calm.', 5, 2, 'minimal', '{}', '{quick, stress-relief, anywhere}', 'wind', true), ('Nature Soundscape', 'sensory', 'Listen to calming nature sounds for sensory regulation.', 15, 3, 'minimal', '{headphones}', '{sensory, calming, audio}', 'tree-pine', true), ('Gentle Stretching', 'physical', 'Light stretching to release physical tension.', 10, 4, 'low', '{comfortable space}', '{movement, tension-relief, physical}', 'sparkles', true), ('Mindful Tea Time', 'restorative', 'Mindful tea preparation and consumption for deep relaxation.', 20, 5, 'low', '{tea, quiet space}', '{mindfulness, ritual, comfort}', 'coffee', true), ('Creative Doodling', 'creative', 'Free-form creative expression to process emotions.', 25, 4, 'low', '{paper, pen/pencils}', '{creative, processing, expression}', 'star', true), ('Guided Meditation', 'mental', 'Structured meditation for deep mental restoration.', 15, 6, 'medium', '{headphones, quiet space}', '{meditation, deep-recovery, mental}', 'moon', true), ('Sensory Bath/Shower', 'sensory', 'Sensory-focused bathing for comprehensive restoration.', 30, 7, 'medium', '{bathroom, preferred temperature}', '{sensory, water, deep-recovery}', 'waves', true); INSERT INTO public.system_settings (category, setting_key, setting_value, description, is_public) VALUES ('rate_limit', 'requests_per_minute', '60', 'Maximum API requests per minute per user', false), ('rate_limit', 'tokens_per_minute', '10000', 'Maximum tokens per minute per user', false), ('security', 'max_failed_logins', '5', 'Maximum failed login attempts before lockout', false), ('security', 'session_timeout_hours', '24', 'User session timeout in hours', false), ('crisis', 'detection_sensitivity', '"medium"', 'Crisis detection sensitivity level', false), ('crisis', 'auto_intervention', 'true', 'Enable automatic crisis interventions', false), ('monitoring', 'enable_metrics', 'true', 'Enable system metrics collection', false), ('monitoring', 'log_level', '"info"', 'System logging level', false), ('general', 'maintenance_mode', 'false', 'System maintenance mode flag', true), ('general', 'feature_flags', '{"recovery_system": true, "crisis_detection": true, "social_battery": true}', 'Feature availability flags', true) ON CONFLICT (category, setting_key) DO NOTHING