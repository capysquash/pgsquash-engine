...
$$; CREATE EXTENSION IF NOT EXISTS cube; CREATE EXTENSION IF NOT EXISTS earthdistance; CREATE EXTENSION IF NOT EXISTS postgis; CREATE EXTENSION IF NOT EXISTS pg_trgm; CREATE EXTENSION IF NOT EXISTS pg_stat_statements; CREATE EXTENSION IF NOT EXISTS btree_gin; CREATE EXTENSION IF NOT EXISTS pgcrypto; CREATE TABLE IF NOT EXISTS faq_content (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), category text NOT NULL, question text NOT NULL, answer text NOT NULL, locale text NOT NULL DEFAULT 'en', tags jsonb DEFAULT '[]', sort_order int DEFAULT 0, is_active boolean DEFAULT true, created_at timestamptz DEFAULT now() NOT NULL, updated_at timestamptz DEFAULT now() NOT NULL, UNIQUE (category, question, locale)); CREATE TABLE IF NOT EXISTS market_configs (name text NOT NULL, currency text NOT NULL, language text NOT NULL, is_active boolean DEFAULT true, time_zone text NOT NULL DEFAULT 'UTC', date_format text NOT NULL DEFAULT 'YYYY-MM-DD', settings jsonb NOT NULL DEFAULT ('{}'::jsonb), created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now(), market_code text PRIMARY KEY, config jsonb DEFAULT ('{}'::jsonb)); CREATE TABLE IF NOT EXISTS countries (code text PRIMARY KEY, name text NOT NULL, flag_emoji text NOT NULL, is_priority_market boolean DEFAULT false, sort_order int DEFAULT 999, is_active boolean DEFAULT true, created_at timestamptz DEFAULT now() NOT NULL, updated_at timestamptz DEFAULT now() NOT NULL); CREATE TYPE user_status AS ENUM ('pending', 'active', 'inactive', 'suspended', 'banned'); DROP VIEW IF EXISTS public_roommate_listings_secure CASCADE; CREATE TYPE conversation_status_enum AS ENUM ('active', 'archived', 'blocked', 'pending_approval'); CREATE TABLE IF NOT EXISTS notification_templates (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), notification_type text NOT NULL, title text NOT NULL, body text NOT NULL, action_url text, icon text, locale text NOT NULL DEFAULT 'en', variables jsonb DEFAULT '[]', is_active boolean DEFAULT true, created_at timestamptz DEFAULT now() NOT NULL, updated_at timestamptz DEFAULT now() NOT NULL, UNIQUE (notification_type, locale)); CREATE TABLE IF NOT EXISTS boost_products (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), name text NOT NULL, boost_code text NOT NULL UNIQUE, price_cents int NOT NULL, duration_days int NOT NULL, boost_type text NOT NULL CHECK (boost_type IN ('profile_highlight', 'top_placement', 'urgent_badge')), features jsonb NOT NULL DEFAULT ('{}'::jsonb), is_active boolean DEFAULT true, stripe_price_id text, created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now()); CREATE TABLE IF NOT EXISTS amenities (id text PRIMARY KEY, label text NOT NULL, icon text NOT NULL, category text NOT NULL CHECK (category IN ('essential', 'comfort', 'lifestyle', 'luxury')), sort_order int DEFAULT 0, is_active boolean DEFAULT true, created_at timestamptz DEFAULT now() NOT NULL, updated_at timestamptz DEFAULT now() NOT NULL); CREATE TABLE IF NOT EXISTS monitoring_config (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), config_key text UNIQUE NOT NULL, config_value jsonb NOT NULL, description text, is_active boolean DEFAULT true, created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now()); CREATE TABLE IF NOT EXISTS lifestyle_preference_options (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), category text NOT NULL CHECK (category IN ('cleanliness', 'social_level', 'noise_level', 'guest_policy', 'smoking_policy', 'pet_policy', 'cooking_frequency', 'kitchen_sharing', 'work_from_home', 'noise_during_work', 'cleaning_frequency', 'chore_sharing', 'conflict_resolution', 'decision_making', 'interests')), value text NOT NULL, label text NOT NULL, description text, icon text, locale text NOT NULL DEFAULT 'en', sort_order int DEFAULT 0, is_active boolean DEFAULT true, created_at timestamptz DEFAULT now() NOT NULL, updated_at timestamptz DEFAULT now() NOT NULL, UNIQUE (category, value, locale)); CREATE TABLE IF NOT EXISTS webhook_deliveries (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), webhook_id uuid NOT NULL, event_type text NOT NULL, payload jsonb NOT NULL, status text DEFAULT 'pending' CHECK (status IN ('pending', 'success', 'failed', 'retrying')), response_code int, response_body text, attempt_count int DEFAULT 0, next_retry_at timestamptz, delivered_at timestamptz, created_at timestamptz DEFAULT now()); CREATE TABLE IF NOT EXISTS system_health_metrics (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), metric_name text NOT NULL, metric_value numeric, threshold_warning numeric, threshold_critical numeric, status text DEFAULT 'normal' CHECK (status IN ('normal', 'warning', 'critical')), recorded_at timestamptz DEFAULT now()); CREATE TYPE verification_status AS ENUM ('pending', 'verified', 'rejected', 'expired'); CREATE TABLE IF NOT EXISTS public.onboarding_steps (id bigserial PRIMARY KEY, persona_type text NOT NULL CHECK (persona_type IN ('room_seeker', 'room_owner', 'student', 'buddy_up', 'expat', 'property_manager', 'neutral')), step_number int NOT NULL, step_key text NOT NULL, title text NOT NULL, subtitle text, description text, help_text text, is_required boolean DEFAULT true, sort_order int DEFAULT 0, locale text DEFAULT 'en' NOT NULL, is_active boolean DEFAULT true, created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now(), UNIQUE (persona_type, step_number, locale), UNIQUE (step_key, locale)); DROP TABLE IF EXISTS tenant_group_members CASCADE; DROP TABLE IF EXISTS tenant_groups CASCADE; CREATE TYPE verification_status_enum AS ENUM ('pending', 'in_progress', 'completed', 'failed', 'rejected'); CREATE TABLE IF NOT EXISTS ai_models (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), model_alias text UNIQUE NOT NULL, provider text NOT NULL CHECK (provider IN ('azure', 'openai', 'anthropic')), model_id text NOT NULL, deployment_name text, is_active boolean DEFAULT true, settings jsonb DEFAULT ('{}'::jsonb), tier_access text[] DEFAULT ARRAY['free', 'premium'], cost_per_1k_prompt numeric(10, 6), cost_per_1k_completion numeric(10, 6), max_output_tokens int DEFAULT 2048, description text, created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now()); CREATE TABLE IF NOT EXISTS public.validation_messages (id bigserial PRIMARY KEY, validation_key text NOT NULL, field_name text, message text NOT NULL, severity text DEFAULT 'error' CHECK (severity IN ('error', 'warning', 'info')), locale text DEFAULT 'en' NOT NULL, is_active boolean DEFAULT true, created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now(), UNIQUE (validation_key, locale)); DROP TABLE IF EXISTS tenant_group_expenses CASCADE; CREATE TABLE IF NOT EXISTS mbti_compatibility_scores (type2 text NOT NULL CHECK (type2 ~ '^[EI][SN][TF][JP]$'), compatibility_score numeric(3, 2) NOT NULL CHECK (compatibility_score >= 0 AND compatibility_score <= 1), dimension_scores jsonb DEFAULT '{}', calculated_at timestamptz DEFAULT now(), algorithm_version text DEFAULT 'v1.0', CHECK (type1 <= type2), id uuid PRIMARY KEY DEFAULT gen_random_uuid(), type1 text NOT NULL CHECK (type1 ~ '^[EI][SN][TF][JP]$'), UNIQUE (type1, type2, algorithm_version)); CREATE TABLE IF NOT EXISTS universities (established_year int, is_partner boolean DEFAULT false, student_discount_percentage int DEFAULT 0 CHECK (student_discount_percentage >= 0 AND student_discount_percentage <= 100), updated_at timestamptz DEFAULT now(), verification_enabled boolean DEFAULT true, email_domain text, country_code text NOT NULL, city text, website_url text, university_type text DEFAULT 'public' CHECK (university_type IN ('public', 'private', 'technical', 'research')), longitude numeric(11, 8), timezone text, logo_url text, created_at timestamptz DEFAULT now(), student_count int, latitude numeric(10, 8), id uuid PRIMARY KEY DEFAULT gen_random_uuid(), name text NOT NULL, is_active boolean DEFAULT true); CREATE TABLE IF NOT EXISTS neighborhoods (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), name text NOT NULL, state text, country varchar(2) NOT NULL DEFAULT 'US', coordinates double precision[], demographics jsonb DEFAULT '{}', safety_rating numeric(3, 2), created_at timestamptz DEFAULT now(), city text NOT NULL, description text, amenities text[], transport_links text[], updated_at timestamptz DEFAULT now(), average_rent_1br numeric(10, 2), metro_stations text[], cost_of_living_rating numeric(3, 2), average_rent_3br numeric(10, 2), average_rent_2br numeric(10, 2), currency text DEFAULT 'EUR', transport_rating numeric(3, 2), nightlife_rating numeric(3, 2), universities text[]); DROP TABLE IF EXISTS tenant_group_budgets CASCADE; CREATE TABLE IF NOT EXISTS subscription_plans (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), name text NOT NULL, plan_code text NOT NULL UNIQUE, price_monthly int, is_active boolean DEFAULT true, stripe_price_id_monthly text, updated_at timestamptz DEFAULT now(), price_annual int, features jsonb NOT NULL DEFAULT ('{}'::jsonb), market_availability text[], stripe_price_id_annual text, created_at timestamptz DEFAULT now(), description text, limits jsonb DEFAULT '{}', is_popular boolean DEFAULT false, sort_order int DEFAULT 0); CREATE TABLE IF NOT EXISTS relocation_services (updated_at timestamptz DEFAULT now(), description text, countries text[] DEFAULT '{}', provider_name text, service_type text NOT NULL CHECK (service_type IN ('bank_account', 'residence_permit', 'sim_card', 'utilities', 'furniture', 'transport', 'insurance', 'language', 'community')), currency text DEFAULT 'EUR', processing_time_days int, created_at timestamptz DEFAULT now(), id uuid PRIMARY KEY DEFAULT gen_random_uuid(), name text NOT NULL, is_partner_service boolean DEFAULT false, discount_percentage int DEFAULT 0, provider_contact text, estimated_cost_min numeric(10, 2), estimated_cost_max numeric(10, 2)); CREATE TABLE IF NOT EXISTS expense_categories (id text PRIMARY KEY, label text NOT NULL, description text, icon text, sort_order int DEFAULT 0, is_active boolean DEFAULT true, created_at timestamptz DEFAULT now() NOT NULL, updated_at timestamptz DEFAULT now() NOT NULL); CREATE TABLE IF NOT EXISTS error_messages (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), error_code text NOT NULL, title text NOT NULL, message text NOT NULL, suggested_action text, locale text NOT NULL DEFAULT 'en', is_active boolean DEFAULT true, created_at timestamptz DEFAULT now() NOT NULL, updated_at timestamptz DEFAULT now() NOT NULL, UNIQUE (error_code, locale)); CREATE TABLE IF NOT EXISTS chat_models (id text PRIMARY KEY, name text NOT NULL, description text NOT NULL, model_id text NOT NULL, max_output_tokens int, temperature numeric(3, 2), is_active boolean DEFAULT true, requires_subscription boolean DEFAULT false, sort_order int DEFAULT 0, created_at timestamptz DEFAULT now() NOT NULL, updated_at timestamptz DEFAULT now() NOT NULL); CREATE TABLE IF NOT EXISTS public.community_categories (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), slug text UNIQUE NOT NULL, name text NOT NULL, description text, icon text, color text, metadata jsonb DEFAULT '{}', sort_order int DEFAULT 0, locale text DEFAULT 'en', active boolean DEFAULT true, created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now(), UNIQUE (slug, locale)); CREATE TABLE IF NOT EXISTS personality_compatibility_cache (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), type1 varchar(4) NOT NULL, type2 varchar(4) NOT NULL, compatibility_score int NOT NULL CHECK (compatibility_score BETWEEN 0 AND 100), calculated_at timestamptz DEFAULT now(), CHECK (type1 <= type2), UNIQUE (type1, type2)); CREATE TABLE IF NOT EXISTS mbti_personality_types (code text PRIMARY KEY CHECK (code ~ '^[EI][NS][FT][JP]$'), name text NOT NULL, description text NOT NULL, strengths jsonb DEFAULT '[]', challenges jsonb DEFAULT '[]', living_style text NOT NULL, ideal_roommate_types jsonb DEFAULT '[]', locale text NOT NULL DEFAULT 'en', is_active boolean DEFAULT true, sort_order int DEFAULT 0, created_at timestamptz DEFAULT now() NOT NULL, updated_at timestamptz DEFAULT now() NOT NULL, UNIQUE (code, locale)); CREATE TABLE IF NOT EXISTS public.ui_text_content (id bigserial PRIMARY KEY, content_key text NOT NULL, content_type text NOT NULL CHECK (content_type IN ('tooltip', 'label', 'placeholder', 'help_text', 'button_text', 'banner', 'toast')), context text, text_content text NOT NULL, locale text DEFAULT 'en' NOT NULL, is_active boolean DEFAULT true, created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now(), UNIQUE (content_key, locale)); CREATE TABLE IF NOT EXISTS monitoring_metrics (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), metric_name text NOT NULL, metric_value numeric NOT NULL, metric_unit text, metric_category text NOT NULL DEFAULT 'system', tags jsonb DEFAULT '{}', threshold_warning numeric, threshold_critical numeric, created_at timestamptz DEFAULT now()); CREATE TYPE notification_status_enum AS ENUM ('pending', 'sent', 'delivered', 'failed', 'read'); CREATE TABLE IF NOT EXISTS persona_options (user_type text PRIMARY KEY CHECK (user_type IN ('room_seeker', 'room_owner', 'student', 'buddy_up', 'expat', 'property_manager')), title text NOT NULL, subtitle text NOT NULL, description text NOT NULL, icon text NOT NULL, benefits jsonb DEFAULT '[]', features jsonb DEFAULT '[]', is_popular boolean DEFAULT false, is_recommended boolean DEFAULT false, sort_order int DEFAULT 0, is_active boolean DEFAULT true, created_at timestamptz DEFAULT now() NOT NULL, updated_at timestamptz DEFAULT now() NOT NULL); CREATE TABLE IF NOT EXISTS currency_rates (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), base_currency text NOT NULL, target_currency text NOT NULL, rate numeric(20, 10) NOT NULL, rate_date date NOT NULL, source text NOT NULL, created_at timestamptz DEFAULT now(), UNIQUE (base_currency, target_currency, rate_date)); CREATE TABLE IF NOT EXISTS mbti_questions (id text PRIMARY KEY, question text NOT NULL, category text NOT NULL CHECK (category IN ('social_living', 'household_management', 'communication', 'lifestyle_choices')), scenario text, dimension text NOT NULL CHECK (dimension IN ('energy', 'information', 'decisions', 'lifestyle')), sort_order int DEFAULT 0, is_active boolean DEFAULT true, created_at timestamptz DEFAULT now() NOT NULL, updated_at timestamptz DEFAULT now() NOT NULL); CREATE TABLE IF NOT EXISTS public.property_options (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), category text NOT NULL, key text NOT NULL, label text NOT NULL, description text, icon text, metadata jsonb DEFAULT '{}', sort_order int DEFAULT 0, locale text DEFAULT 'en', active boolean DEFAULT true, created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now(), UNIQUE (category, key, locale)); CREATE TABLE IF NOT EXISTS public.onboarding_question_options (id bigserial PRIMARY KEY, question_key text NOT NULL, option_value text NOT NULL, option_label text NOT NULL, option_description text, icon_name text, sort_order int DEFAULT 0, locale text DEFAULT 'en' NOT NULL, is_active boolean DEFAULT true, created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now(), UNIQUE (question_key, option_value, locale)); CREATE TABLE IF NOT EXISTS public.report_reasons (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), category text NOT NULL, key text NOT NULL, label text NOT NULL, description text, severity text DEFAULT 'medium', requires_details boolean DEFAULT false, auto_action text, metadata jsonb DEFAULT '{}', sort_order int DEFAULT 0, locale text DEFAULT 'en', active boolean DEFAULT true, created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now(), UNIQUE (category, key, locale)); CREATE TABLE IF NOT EXISTS ai_usage_limits (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), tier text NOT NULL, limit_type text NOT NULL, limit_value int NOT NULL, reset_period text DEFAULT 'daily' CHECK (reset_period IN ('daily', 'monthly', 'never')), created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now(), is_active boolean DEFAULT true NOT NULL); CREATE TABLE IF NOT EXISTS market_metrics (market_code text REFERENCES market_configs (market_code) ON DELETE CASCADE, date date NOT NULL, users_count int DEFAULT 0 NOT NULL, messages_count int DEFAULT 0 NOT NULL, bookings_count int DEFAULT 0 NOT NULL, revenue numeric(10, 2) DEFAULT 0 NOT NULL, id uuid PRIMARY KEY DEFAULT gen_random_uuid(), properties_count int DEFAULT 0 NOT NULL, active_listings_count int DEFAULT 0 NOT NULL, matches_count int DEFAULT 0 NOT NULL, gmv numeric(10, 2) DEFAULT 0 NOT NULL, created_at timestamptz DEFAULT now(), UNIQUE (market_code, date)); CREATE TABLE IF NOT EXISTS market_legal_documents (updated_at timestamptz DEFAULT now(), market_code text REFERENCES market_configs (market_code) ON DELETE CASCADE, document_type text NOT NULL CHECK (document_type IN ('terms_of_service', 'privacy_policy', 'rental_agreement', 'house_rules', 'cancellation_policy')), content text NOT NULL, version text NOT NULL, effective_date date NOT NULL, language text NOT NULL DEFAULT 'en', created_at timestamptz DEFAULT now(), id uuid PRIMARY KEY DEFAULT gen_random_uuid(), title text NOT NULL, is_active boolean DEFAULT true, UNIQUE (market_code, document_type, language, version)); CREATE TABLE IF NOT EXISTS profiles (field_of_study text, is_verified boolean DEFAULT false, avatar_url text, rating_count int DEFAULT 0, onboarding_completed boolean DEFAULT false, destination_country varchar(2), auth_provider text DEFAULT 'clerk' CHECK (auth_provider = 'clerk'), age int CHECK (age BETWEEN 18 AND 100), last_active timestamptz DEFAULT now(), business_type text, preferred_locale text DEFAULT 'en', relocation_status text CHECK (relocation_status IN ('planning', 'in_progress', 'completed')), student_id text, portfolio_size_limit int DEFAULT 5, updated_at timestamptz DEFAULT now(), name text, last_name text, gender text CHECK (gender IN ('male', 'female', 'non_binary', 'other', 'prefer_not_to_say')), home_country varchar(2), current_country varchar(2), university text, email text NOT NULL UNIQUE, bio text, email_verified boolean DEFAULT false, company_name text, managed_properties_count int DEFAULT 0, department text, role text DEFAULT 'user' CHECK (role IN ('user', 'property_manager', 'admin', 'super_admin', 'translator')), first_name text, occupation text, status text DEFAULT 'active' CHECK (status IN ('active', 'inactive', 'banned', 'suspended')), coordinates double precision[], mbti_type varchar(4), rating_average numeric(3, 2) DEFAULT 0 CHECK (rating_average BETWEEN 0 AND 5), id text PRIMARY KEY, verification_status text DEFAULT 'pending' CHECK (verification_status IN ('pending', 'verified', 'rejected')), user_type text CHECK (user_type IN ('room_seeker', 'room_owner', 'buddy_up', 'property_manager', 'student', 'expat', 'real_estate_pro')), relocation_date date, license_number text, property_management_scope text DEFAULT 'individual' CHECK (property_management_scope IN ('individual', 'professional', 'enterprise')), metadata jsonb DEFAULT '{}', created_at timestamptz DEFAULT now(), graduation_year int, notification_preferences jsonb DEFAULT ('{
    ...
    }'::jsonb), city text, rejection_reason text, phone text, privacy_settings jsonb DEFAULT ('{
        "public_profile": true,
        "show_age": true,
        "show_email": false,
        "show_phone": false,
...
END $$; CREATE OR REPLACE FUNCTION generate_property_slug() RETURNS trigger LANGUAGE plpgsql VOLATILE AS $$
BEGIN
...
BEGIN
    ...
    RETURN request_id;
END;
$$; CREATE TRIGGER update_mbti_questions_updated_at BEFORE UPDATE ON mbti_questions FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_onboarding_question_options_updated_at BEFORE UPDATE ON public.onboarding_question_options FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_profiles_updated_at BEFORE UPDATE ON profiles FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER business_intelligence_cache_updated_at BEFORE UPDATE ON business_intelligence_cache FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER compliance_reports_updated_at BEFORE UPDATE ON compliance_reports FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_onboarding_steps_updated_at BEFORE UPDATE ON public.onboarding_steps FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_saved_roommates_updated_at BEFORE UPDATE ON saved_roommates FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER user_subscriptions_updated_at BEFORE UPDATE ON user_subscriptions FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER maintenance_requests_updated_at BEFORE UPDATE ON maintenance_requests FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_amenities_updated_at BEFORE UPDATE ON amenities FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER rooms_updated_at BEFORE UPDATE ON rooms FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER trigger_update_profile_verified_status AFTER INSERT OR UPDATE ON user_badges FOR EACH ROW EXECUTE FUNCTION update_profile_verified_status(); CREATE TRIGGER update_faq_content_updated_at BEFORE UPDATE ON faq_content FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER saved_searches_updated_at BEFORE UPDATE ON saved_searches FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER feature_flags_updated_at BEFORE UPDATE ON feature_flags FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER tenant_screening_results_updated_at BEFORE UPDATE ON tenant_screening_results FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER set_updated_at_verification_flows BEFORE UPDATE ON verification_flows FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at(); CREATE TRIGGER enterprise_relocations_updated_at BEFORE UPDATE ON enterprise_relocations FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER property_multimedia_updated_at BEFORE UPDATE ON property_multimedia FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER set_updated_at_user_notifications BEFORE UPDATE ON user_notifications FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at(); CREATE TRIGGER university_email_domains_updated_at BEFORE UPDATE ON university_email_domains FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_deposit_insurance_updated_at BEFORE UPDATE ON deposit_insurance FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER market_legal_documents_updated_at BEFORE UPDATE ON market_legal_documents FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER payment_transactions_updated_at BEFORE UPDATE ON payment_transactions FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_ai_chats_updated_at BEFORE UPDATE ON ai_chats FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER check_utilities_logic_rooms BEFORE INSERT OR UPDATE ON rooms FOR EACH ROW WHEN (new.utilities_included IS NOT NULL) EXECUTE FUNCTION validate_utilities_logic(); CREATE TRIGGER set_updated_at_buddy_connection_members BEFORE UPDATE ON buddy_connection_members FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at(); CREATE TRIGGER trigger_update_managed_properties_count AFTER INSERT OR DELETE ON properties FOR EACH ROW EXECUTE FUNCTION update_managed_properties_count(); CREATE TRIGGER update_fairrent_room_scores_updated_at BEFORE UPDATE ON fairrent_room_scores FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER universities_updated_at BEFORE UPDATE ON universities FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER set_updated_at_match_metrics BEFORE UPDATE ON match_metrics FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at(); CREATE TRIGGER matches_updated_at BEFORE UPDATE ON matches FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_persona_options_updated_at BEFORE UPDATE ON persona_options FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_chat_models_updated_at BEFORE UPDATE ON chat_models FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER financial_forecasts_updated_at BEFORE UPDATE ON financial_forecasts FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER api_keys_updated_at BEFORE UPDATE ON api_keys FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_safety_scores_on_rating AFTER INSERT OR UPDATE ON user_reviews FOR EACH ROW EXECUTE FUNCTION trigger_safety_score_update(); CREATE TRIGGER expense_splits_updated_at BEFORE UPDATE ON expense_splits FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_fairrent_scores_updated_at BEFORE UPDATE ON fairrent_scores FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER ai_chats_updated_at BEFORE UPDATE ON ai_chats FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER security_deposits_updated_at BEFORE UPDATE ON security_deposits FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER market_configs_updated_at BEFORE UPDATE ON market_configs FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER validate_profile_trigger BEFORE INSERT OR UPDATE ON profiles FOR EACH ROW EXECUTE FUNCTION validate_profile_data(); CREATE TRIGGER update_roommate_listings_updated_at BEFORE UPDATE ON roommate_listings FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_user_preferences_updated_at BEFORE UPDATE ON user_preferences FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_lifestyle_preference_options_updated_at BEFORE UPDATE ON lifestyle_preference_options FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER user_personality_results_updated_at BEFORE UPDATE ON user_personality_results FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER community_posts_updated_at BEFORE UPDATE ON community_posts FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER properties_updated_at BEFORE UPDATE ON properties FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER notification_preferences_updated_at BEFORE UPDATE ON notification_preferences FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER set_updated_at_calendar_events BEFORE UPDATE ON calendar_events FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at(); CREATE TRIGGER set_updated_at_platform_settings BEFORE UPDATE ON platform_settings FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at(); CREATE TRIGGER update_error_messages_updated_at BEFORE UPDATE ON error_messages FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER marketing_campaigns_updated_at BEFORE UPDATE ON marketing_campaigns FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_onboarding_questions_updated_at BEFORE UPDATE ON public.onboarding_questions FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_buddy_connections_updated_at BEFORE UPDATE ON buddy_connections FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_buddy_connection_members_updated_at BEFORE UPDATE ON buddy_connection_members FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_report_templates_updated_at BEFORE UPDATE ON report_templates FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER student_verifications_updated_at BEFORE UPDATE ON student_verifications FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER user_verification_documents_updated_at BEFORE UPDATE ON user_verification_documents FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER set_updated_at_buddy_connections BEFORE UPDATE ON buddy_connections FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at(); CREATE TRIGGER roommate_listings_updated_at BEFORE UPDATE ON roommate_listings FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER trigger_set_property_management_scope BEFORE INSERT OR UPDATE OF user_type ON profiles FOR EACH ROW EXECUTE FUNCTION set_property_management_scope(); CREATE TRIGGER relocation_services_updated_at BEFORE UPDATE ON relocation_services FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER translations_updated_at BEFORE UPDATE ON translations FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER community_events_updated_at BEFORE UPDATE ON community_events FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_properties_updated_at BEFORE UPDATE ON properties FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER set_updated_at_conversation_requests BEFORE UPDATE ON conversation_requests FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at(); CREATE TRIGGER gdpr_requests_updated_at BEFORE UPDATE ON gdpr_requests FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_conversations_updated_at BEFORE UPDATE ON conversations FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); DROP TRIGGER IF EXISTS prevent_null_fairrent_fields_trigger ON fairrent_scores; CREATE TRIGGER set_updated_at_monitoring_config BEFORE UPDATE ON monitoring_config FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at(); CREATE TRIGGER update_conversation_on_new_message AFTER INSERT ON messages FOR EACH ROW EXECUTE FUNCTION update_unread_counts(); CREATE TRIGGER saved_properties_updated_at BEFORE UPDATE ON saved_properties FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER user_preferences_updated_at BEFORE UPDATE ON user_preferences FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_mass_message_campaigns_updated_at BEFORE UPDATE ON mass_message_campaigns FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_validation_messages_updated_at BEFORE UPDATE ON public.validation_messages FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_countries_updated_at BEFORE UPDATE ON countries FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER country_business_rules_updated_at BEFORE UPDATE ON country_business_rules FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER trigger_generate_profile_slug BEFORE INSERT OR UPDATE ON profiles FOR EACH ROW EXECUTE FUNCTION generate_profile_slug(); CREATE TRIGGER create_calendar_event_trigger AFTER INSERT OR UPDATE ON bookings FOR EACH ROW EXECUTE FUNCTION create_calendar_event_from_booking(); CREATE TRIGGER screening_questionnaires_updated_at BEFORE UPDATE ON screening_questionnaires FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER user_relocation_requests_updated_at BEFORE UPDATE ON user_relocation_requests FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_messages_updated_at BEFORE UPDATE ON messages FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_ai_models_updated_at BEFORE UPDATE ON ai_models FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_ai_usage_limits_updated_at BEFORE UPDATE ON ai_usage_limits FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER profiles_updated_at BEFORE UPDATE ON profiles FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_notification_templates_updated_at BEFORE UPDATE ON notification_templates FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER subscription_plans_updated_at BEFORE UPDATE ON subscription_plans FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER country_content_updated_at BEFORE UPDATE ON country_content FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER check_utilities_logic_properties BEFORE INSERT OR UPDATE ON properties FOR EACH ROW WHEN (new.utilities_included IS NOT NULL) EXECUTE FUNCTION validate_utilities_logic(); CREATE TRIGGER property_options_updated_at BEFORE UPDATE ON public.property_options FOR EACH ROW EXECUTE FUNCTION update_property_options_updated_at(); CREATE TRIGGER university_partnerships_updated_at BEFORE UPDATE ON university_partnerships FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER set_updated_at_report_requests BEFORE UPDATE ON report_requests FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at(); CREATE TRIGGER set_updated_at_general_conversations BEFORE UPDATE ON general_conversations FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at(); CREATE TRIGGER set_updated_at_messaging_preferences BEFORE UPDATE ON messaging_preferences FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at(); CREATE TRIGGER report_reasons_updated_at BEFORE UPDATE ON public.report_reasons FOR EACH ROW EXECUTE FUNCTION update_report_reasons_updated_at(); CREATE TRIGGER community_categories_updated_at BEFORE UPDATE ON public.community_categories FOR EACH ROW EXECUTE FUNCTION update_community_categories_updated_at(); CREATE TRIGGER update_ai_usage_tracking_updated_at BEFORE UPDATE ON ai_usage_tracking FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER neighborhoods_updated_at BEFORE UPDATE ON neighborhoods FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_expense_categories_updated_at BEFORE UPDATE ON expense_categories FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER trigger_generate_property_slug BEFORE INSERT OR UPDATE ON properties FOR EACH ROW EXECUTE FUNCTION generate_property_slug(); CREATE TRIGGER trigger_update_compatibility_updated_at BEFORE UPDATE ON user_compatibility_scores FOR EACH ROW EXECUTE FUNCTION update_compatibility_updated_at(); CREATE TRIGGER update_ui_text_content_updated_at BEFORE UPDATE ON public.ui_text_content FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER personality_test_trigger AFTER INSERT ON user_personality_results FOR EACH ROW EXECUTE FUNCTION update_user_preferences_from_personality(); CREATE TRIGGER expense_shares_updated_at BEFORE UPDATE ON expense_shares FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_mbti_personality_types_updated_at BEFORE UPDATE ON mbti_personality_types FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER update_enterprise_webhooks_updated_at BEFORE UPDATE ON enterprise_webhooks FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER enterprise_settings_updated_at BEFORE UPDATE ON enterprise_settings FOR EACH ROW EXECUTE FUNCTION update_updated_at_column(); CREATE TRIGGER enforce_fairrent_required_fields BEFORE INSERT OR UPDATE ON properties FOR EACH ROW EXECUTE FUNCTION prevent_null_fairrent_fields(); CREATE TRIGGER trigger_generate_roommate_listing_slug BEFORE INSERT OR UPDATE ON roommate_listings FOR EACH ROW EXECUTE FUNCTION generate_roommate_listing_slug(); CREATE INDEX IF NOT EXISTS idx_profiles_last_active ON profiles USING btree (last_active); CREATE INDEX IF NOT EXISTS idx_notification_queue_priority ON notification_queue USING btree (priority); CREATE INDEX IF NOT EXISTS idx_profiles_notification_preferences ON profiles USING gin (notification_preferences); CREATE INDEX IF NOT EXISTS idx_university_email_domains_domain ON university_email_domains USING btree (domain); CREATE INDEX IF NOT EXISTS idx_buddy_connection_members_connection_id ON buddy_connection_members USING btree (buddy_connection_id); CREATE INDEX IF NOT EXISTS idx_calendar_events_user_id ON calendar_events USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_countries_is_priority_market ON countries USING btree (is_priority_market) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_mbti_questions_dimension ON mbti_questions USING btree (dimension) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_properties_available_from ON properties USING btree (available_from) WHERE available_from IS NOT NULL AND is_active = true; CREATE INDEX IF NOT EXISTS idx_error_messages_locale ON error_messages USING btree (locale) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_match_metrics_match_id ON match_metrics USING btree (match_id); CREATE INDEX IF NOT EXISTS idx_conversations_match_id ON conversations USING btree (match_id) WHERE match_id IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_community_posts_created_at ON community_posts USING btree (created_at); CREATE INDEX IF NOT EXISTS idx_profiles_portfolio_size ON profiles USING btree (portfolio_size) WHERE portfolio_size IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_user_reports_reported_user ON user_reports USING btree (reported_user_id); CREATE INDEX IF NOT EXISTS idx_report_requests_status ON report_requests USING btree (status); CREATE INDEX IF NOT EXISTS idx_roommate_listings_created_at ON roommate_listings USING btree (created_at); CREATE INDEX IF NOT EXISTS idx_ai_chat_votes_message_id ON ai_chat_votes USING btree (message_id); CREATE INDEX IF NOT EXISTS idx_messages_conversation ON messages USING btree (conversation_id, created_at DESC); CREATE INDEX IF NOT EXISTS idx_properties_owner_id ON properties USING btree (owner_id); CREATE INDEX IF NOT EXISTS idx_expense_shares_user_id ON expense_shares USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_community_memberships_user ON community_memberships USING btree (user_id, community_id); CREATE INDEX IF NOT EXISTS idx_user_notifications_user_id ON user_notifications USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_user_subscriptions_type_status ON user_subscriptions USING btree (subscription_type, status, end_date); CREATE INDEX IF NOT EXISTS idx_ai_chat_messages_role ON ai_chat_messages USING btree (role); CREATE INDEX IF NOT EXISTS idx_report_requests_user_id ON report_requests USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_mbti_assessments_user_latest ON mbti_assessments USING btree (user_id, completed_at DESC); CREATE INDEX IF NOT EXISTS idx_profiles_username ON profiles USING btree (username) WHERE username IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_notifications_unread ON notifications USING btree (recipient_id, read_at) WHERE read_at IS NULL; CREATE INDEX IF NOT EXISTS idx_deposit_insurance_policy_number ON deposit_insurance USING btree (policy_number); CREATE INDEX IF NOT EXISTS idx_faq_content_locale ON faq_content USING btree (locale) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_profiles_department ON profiles USING btree (department) WHERE department IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_api_usage_logs_org ON api_usage_logs USING btree (organization_id); CREATE INDEX IF NOT EXISTS idx_report_reasons_active ON public.report_reasons USING btree (active); CREATE INDEX IF NOT EXISTS idx_notification_queue_channel ON notification_queue USING btree (channel); CREATE INDEX IF NOT EXISTS idx_analytics_events_category ON analytics_events USING btree (event_category); CREATE INDEX IF NOT EXISTS idx_verification_flows_user_id ON verification_flows USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_mass_message_campaigns_created_by ON mass_message_campaigns USING btree (created_by); CREATE INDEX IF NOT EXISTS idx_ai_models_provider ON ai_models USING btree (provider); CREATE INDEX IF NOT EXISTS idx_conversation_requests_recipient ON conversation_requests USING btree (recipient_id); CREATE INDEX IF NOT EXISTS idx_rooms_fairrent_ready ON rooms USING btree (id) WHERE furnishing_status IS NOT NULL AND utilities_included IS NOT NULL AND status = 'available'; CREATE INDEX IF NOT EXISTS idx_email_templates_type ON email_templates USING btree (template_type) WHERE is_active = true; CREATE INDEX idx_user_compat_scores_user2_score ON user_compatibility_scores USING btree (user2_id, compatibility_score DESC); CREATE INDEX IF NOT EXISTS idx_universities_is_partner ON universities USING btree (is_partner); CREATE INDEX IF NOT EXISTS idx_ai_models_active ON ai_models USING btree (is_active); CREATE INDEX IF NOT EXISTS idx_currency_rates_base_target_date ON currency_rates USING btree (base_currency, target_currency, rate_date DESC); CREATE INDEX IF NOT EXISTS idx_ai_routing_rules_priority ON ai_routing_rules USING btree (priority DESC); CREATE INDEX IF NOT EXISTS idx_notification_queue_user_status ON notification_queue USING btree (user_id, status); CREATE INDEX IF NOT EXISTS idx_notification_templates_type ON notification_templates USING btree (notification_type) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_bookings_user_status ON bookings USING btree (user_id, status); CREATE INDEX idx_fairrent_scores_calculated_at ON fairrent_scores USING btree (calculated_at DESC); CREATE INDEX IF NOT EXISTS idx_properties_price_range ON properties USING btree (price, bedrooms) WHERE status = 'available'; CREATE INDEX IF NOT EXISTS idx_buddy_connection_members_status ON buddy_connection_members USING btree (status); CREATE INDEX IF NOT EXISTS idx_properties_coordinates ON properties USING btree (coordinates); CREATE INDEX IF NOT EXISTS idx_profiles_coordinates ON profiles USING btree (coordinates) WHERE coordinates IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_buddy_connection_members_user_id ON buddy_connection_members USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_fairrent_room_scores_calculated_at ON fairrent_room_scores USING btree (calculated_at DESC); CREATE INDEX IF NOT EXISTS idx_properties_market_code ON properties USING btree (market_code); CREATE INDEX IF NOT EXISTS idx_matches_status ON matches USING btree (status); CREATE INDEX IF NOT EXISTS idx_platform_settings_category_key ON platform_settings USING btree (category, setting_key); CREATE INDEX IF NOT EXISTS idx_notification_templates_locale ON notification_templates USING btree (locale) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_ai_usage_limits_tier ON ai_usage_limits USING btree (tier); CREATE INDEX IF NOT EXISTS idx_user_reports_created_at ON user_reports USING btree (created_at DESC); CREATE INDEX IF NOT EXISTS idx_user_actions_action ON user_actions USING btree (action); CREATE INDEX IF NOT EXISTS idx_user_reviews_reviewee ON user_reviews USING btree (reviewee_id, created_at DESC); CREATE INDEX IF NOT EXISTS idx_matches_compatibility ON matches USING btree (compatibility_score); CREATE UNIQUE INDEX IF NOT EXISTS idx_conversations_participants_unique ON conversations USING btree (LEAST(participant_1_id, participant_2_id), GREATEST(participant_1_id, participant_2_id)); CREATE INDEX IF NOT EXISTS idx_api_usage_logs_api_key ON api_usage_logs USING btree (api_key_id); CREATE INDEX IF NOT EXISTS idx_properties_manager_id ON properties USING btree (manager_id); CREATE INDEX IF NOT EXISTS idx_properties_status ON properties USING btree (status); CREATE INDEX IF NOT EXISTS idx_profiles_company_name ON profiles USING btree (company_name) WHERE company_name IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_conversation_requests_status ON conversation_requests USING btree (status); CREATE INDEX IF NOT EXISTS idx_platform_settings_updated_at ON platform_settings USING btree (updated_at DESC); CREATE INDEX IF NOT EXISTS idx_profiles_slug ON profiles USING btree (slug); CREATE INDEX IF NOT EXISTS idx_notifications_recipient_unread ON notifications USING btree (recipient_id, created_at DESC) WHERE read_at IS NULL; CREATE INDEX IF NOT EXISTS idx_communities_creator_id ON communities USING btree (creator_id); CREATE INDEX IF NOT EXISTS idx_expense_splits_property_id ON expense_splits USING btree (property_id); CREATE INDEX IF NOT EXISTS idx_monitoring_config_active ON monitoring_config USING btree (is_active) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_subscription_plans_sort_order ON subscription_plans USING btree (sort_order) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_verification_flows_document_type ON verification_flows USING btree (document_type); CREATE INDEX IF NOT EXISTS idx_property_options_active ON public.property_options USING btree (active); CREATE INDEX IF NOT EXISTS idx_general_conversations_last_message ON general_conversations USING btree (last_message_at DESC NULLS LAST); CREATE INDEX IF NOT EXISTS idx_expense_categories_sort_order ON expense_categories USING btree (sort_order) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_roommate_listings_type ON roommate_listings USING btree (listing_type); CREATE INDEX IF NOT EXISTS idx_mass_message_campaigns_status ON mass_message_campaigns USING btree (status); CREATE INDEX IF NOT EXISTS idx_matches_user2 ON matches USING btree (user2_id, created_at); CREATE INDEX IF NOT EXISTS idx_country_content_lookup ON country_content USING btree (country, content_type, content_key, locale); CREATE INDEX IF NOT EXISTS idx_roommate_listings_user_id ON roommate_listings USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_bookings_user_room ON bookings USING btree (user_id, room_id); CREATE INDEX IF NOT EXISTS idx_ai_chats_created_at ON ai_chats USING btree (created_at DESC); CREATE INDEX IF NOT EXISTS idx_rooms_fairrent_expires ON rooms USING btree (fairrent_expires_at) WHERE fairrent_score IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_profiles_messaging_mode ON profiles USING btree (messaging_mode) WHERE messaging_mode IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_community_memberships_community_id ON community_memberships USING btree (community_id); CREATE INDEX IF NOT EXISTS idx_notifications_property_management ON notifications USING btree (category, recipient_id, created_at DESC) WHERE category IN ('property_interest', 'viewing_request', 'property_management', 'maintenance_request'); CREATE INDEX IF NOT EXISTS idx_payment_transactions_recipient_id ON payment_transactions USING btree (recipient_id); CREATE INDEX IF NOT EXISTS idx_property_assignments_interest ON property_assignments USING btree (property_interest_id); CREATE INDEX IF NOT EXISTS idx_notifications_system_type ON notifications USING btree (category, created_at DESC) WHERE category IN ('system', 'admin'); CREATE INDEX IF NOT EXISTS idx_persona_options_sort_order ON persona_options USING btree (sort_order) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_properties_fairrent_expires ON properties USING btree (fairrent_expires_at) WHERE fairrent_score IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_analytics_events_page_path ON analytics_events USING btree (page_path); CREATE INDEX IF NOT EXISTS idx_expense_shares_status ON expense_shares USING btree (status); CREATE INDEX idx_fairrent_scores_property_id ON fairrent_scores USING btree (property_id); CREATE INDEX IF NOT EXISTS idx_user_actions_user_id ON user_actions USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_lifestyle_options_sort ON lifestyle_preference_options USING btree (category, sort_order) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_calendar_events_property_id ON calendar_events USING btree (property_id) WHERE property_id IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_viewing_availability_date ON viewing_availability_slots USING btree (property_id, slot_date, is_available); CREATE INDEX IF NOT EXISTS idx_properties_city_country ON properties USING btree (city, country); CREATE INDEX idx_fairrent_scores_property_expires ON fairrent_scores USING btree (property_id, expires_at DESC); CREATE INDEX IF NOT EXISTS idx_properties_price ON properties USING btree (price); CREATE INDEX IF NOT EXISTS idx_calendar_events_type_status ON calendar_events USING btree (event_type, status); CREATE INDEX IF NOT EXISTS idx_ai_chats_user_id ON ai_chats USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_properties_status_active ON properties USING btree (status, is_active) WHERE status = 'available' AND is_active = true; CREATE INDEX IF NOT EXISTS idx_webhook_deliveries_webhook_id ON webhook_deliveries USING btree (webhook_id); CREATE INDEX IF NOT EXISTS idx_profiles_avatar_url ON profiles USING btree (avatar_url) WHERE avatar_url IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_report_requests_created_at ON report_requests USING btree (created_at DESC); CREATE INDEX IF NOT EXISTS idx_ai_chat_votes_user_id ON ai_chat_votes USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_student_verifications_user_id ON student_verifications USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_mbti_personality_types_code ON mbti_personality_types USING btree (code) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_verification_documents_status ON user_verification_documents USING btree (verification_status); CREATE INDEX IF NOT EXISTS idx_webhook_deliveries_status ON webhook_deliveries USING btree (status); CREATE INDEX IF NOT EXISTS idx_profiles_phone ON profiles USING btree (phone); CREATE INDEX IF NOT EXISTS idx_platform_settings_public ON platform_settings USING btree (is_public) WHERE is_public = true; CREATE INDEX IF NOT EXISTS idx_match_metrics_user_id ON match_metrics USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_ai_chat_files_chat_id ON ai_chat_files USING btree (chat_id); CREATE INDEX IF NOT EXISTS idx_ai_chat_files_created_at ON ai_chat_files USING btree (created_at DESC); CREATE INDEX IF NOT EXISTS idx_student_verifications_status ON student_verifications USING btree (verification_status); CREATE INDEX IF NOT EXISTS idx_calendar_events_end_date ON calendar_events USING btree (end_date); CREATE INDEX IF NOT EXISTS idx_matches_users ON matches USING btree (user1_id, user2_id); CREATE INDEX IF NOT EXISTS idx_countries_sort_order ON countries USING btree (sort_order) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_blocked_users_blocked_user_id ON blocked_users USING btree (blocked_user_id); CREATE INDEX IF NOT EXISTS idx_subscription_plans_is_popular ON subscription_plans USING btree (is_popular) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_properties_location ON properties USING btree (city, country); CREATE INDEX IF NOT EXISTS idx_properties_search_optimized ON properties USING btree (is_active, verification_status, city, price, bedrooms) WHERE is_active = true AND verification_status = 'verified'; CREATE INDEX IF NOT EXISTS idx_properties_property_type ON properties USING btree (property_type); CREATE INDEX IF NOT EXISTS idx_buddy_connection_members_connection ON buddy_connection_members USING btree (buddy_connection_id); CREATE INDEX IF NOT EXISTS idx_ai_routing_rules_active ON ai_routing_rules USING btree (is_active); CREATE INDEX IF NOT EXISTS idx_properties_year_built ON properties USING btree (year_built) WHERE year_built IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_user_subscriptions_stripe_customer_id ON user_subscriptions USING btree (stripe_customer_id); CREATE INDEX IF NOT EXISTS idx_conversations_type ON conversations USING btree (conversation_type); CREATE INDEX IF NOT EXISTS idx_ai_chat_files_type ON ai_chat_files USING btree (type); CREATE INDEX IF NOT EXISTS idx_properties_is_active ON properties USING btree (is_active); CREATE INDEX IF NOT EXISTS idx_conversations_participants ON conversations USING btree (participant_1_id, participant_2_id); CREATE INDEX IF NOT EXISTS idx_universities_name ON universities USING btree (name); CREATE INDEX idx_user_compat_scores_user1_score ON user_compatibility_scores USING btree (user1_id, compatibility_score DESC); CREATE INDEX IF NOT EXISTS idx_ai_usage_tracking_user_date ON ai_usage_tracking USING btree (user_id, date); CREATE INDEX IF NOT EXISTS idx_conversation_requests_sender ON conversation_requests USING btree (sender_id); CREATE INDEX IF NOT EXISTS idx_general_conversations_type ON general_conversations USING btree (conversation_type); CREATE INDEX IF NOT EXISTS idx_rooms_status ON rooms USING btree (status); CREATE INDEX IF NOT EXISTS idx_ai_chat_messages_created_at ON ai_chat_messages USING btree (created_at DESC); CREATE INDEX IF NOT EXISTS idx_general_conversations_status ON general_conversations USING btree (status); CREATE INDEX IF NOT EXISTS idx_faq_content_category ON faq_content USING btree (category) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_buddy_connection_members_user ON buddy_connection_members USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_user_subscriptions_user_status ON user_subscriptions USING btree (user_id, status); CREATE INDEX IF NOT EXISTS idx_user_activity_logs_user_time ON user_activity_logs USING btree (user_id, created_at DESC); CREATE INDEX IF NOT EXISTS idx_match_metrics_last_activity ON match_metrics USING btree (last_activity DESC); CREATE INDEX IF NOT EXISTS idx_enterprise_webhooks_org ON enterprise_webhooks USING btree (organization_id); CREATE INDEX IF NOT EXISTS idx_bookings_status ON bookings USING btree (status); CREATE INDEX IF NOT EXISTS idx_ui_text_content_key ON public.ui_text_content USING btree (content_key, locale); CREATE INDEX IF NOT EXISTS idx_conversations_last_message ON conversations USING btree (last_message_at DESC); CREATE INDEX IF NOT EXISTS idx_report_reasons_category ON public.report_reasons USING btree (category); CREATE INDEX IF NOT EXISTS idx_analytics_events_type ON analytics_events USING btree (event_type); CREATE INDEX IF NOT EXISTS idx_properties_address_trgm ON properties USING gin (address gin_trgm_ops); CREATE INDEX IF NOT EXISTS idx_general_conversations_participant_1 ON general_conversations USING btree (participant_1_id); CREATE INDEX IF NOT EXISTS idx_ai_chat_messages_chat_id ON ai_chat_messages USING btree (chat_id); CREATE INDEX IF NOT EXISTS idx_onboarding_steps_persona ON public.onboarding_steps USING btree (persona_type, is_active); CREATE INDEX IF NOT EXISTS idx_bookings_room_id ON bookings USING btree (room_id); CREATE INDEX IF NOT EXISTS idx_messaging_preferences_allow_messages ON messaging_preferences USING btree (allow_messages_from); CREATE INDEX IF NOT EXISTS idx_rooms_price ON rooms USING btree (price); CREATE INDEX IF NOT EXISTS idx_file_uploads_user_context ON file_uploads USING btree (user_id, upload_context); CREATE INDEX IF NOT EXISTS idx_conversations_status ON conversations USING btree (status); CREATE INDEX IF NOT EXISTS idx_analytics_events_user_id ON analytics_events USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_messages_deleted ON messages USING btree (deleted_at) WHERE deleted_at IS NULL; CREATE INDEX IF NOT EXISTS idx_lifestyle_options_locale ON lifestyle_preference_options USING btree (locale) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_messages_reply_to ON messages USING btree (reply_to_id) WHERE reply_to_id IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_community_categories_slug ON public.community_categories USING btree (slug); CREATE INDEX IF NOT EXISTS idx_report_reasons_severity ON public.report_reasons USING btree (severity); CREATE INDEX IF NOT EXISTS idx_fairrent_room_scores_model_version ON fairrent_room_scores USING btree (model_version); CREATE INDEX idx_fairrent_scores_expires_at ON fairrent_scores USING btree (expires_at); CREATE INDEX IF NOT EXISTS idx_calendar_events_room_id ON calendar_events USING btree (room_id) WHERE room_id IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_matches_user1 ON matches USING btree (user1_id, created_at); CREATE INDEX IF NOT EXISTS idx_bookings_room_dates ON bookings USING btree (room_id, start_date, end_date); CREATE INDEX IF NOT EXISTS idx_deposit_insurance_security_deposit_id ON deposit_insurance USING btree (security_deposit_id); CREATE INDEX IF NOT EXISTS idx_notification_queue_scheduled ON notification_queue USING btree (scheduled_for) WHERE status = 'pending'; CREATE INDEX IF NOT EXISTS idx_analytics_events_created_at ON analytics_events USING btree (created_at DESC); CREATE INDEX IF NOT EXISTS idx_community_categories_active ON public.community_categories USING btree (active); CREATE INDEX IF NOT EXISTS idx_universities_country_code ON universities USING btree (country_code); CREATE INDEX IF NOT EXISTS idx_profiles_full_name ON profiles USING btree (full_name) WHERE full_name IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_monitoring_metrics_created_at ON monitoring_metrics USING btree (created_at DESC); CREATE INDEX IF NOT EXISTS idx_roommate_listings_move_in_date ON roommate_listings USING btree (move_in_date); CREATE INDEX IF NOT EXISTS idx_verification_flows_status ON verification_flows USING btree (status); CREATE INDEX IF NOT EXISTS idx_notifications_recipient ON notifications USING btree (recipient_id, created_at); CREATE INDEX IF NOT EXISTS idx_roommate_listings_location_gin ON roommate_listings USING gin (location_preferences); CREATE INDEX IF NOT EXISTS idx_onboarding_questions_step ON public.onboarding_questions USING btree (step_key, is_active); CREATE INDEX IF NOT EXISTS idx_viewing_requests_property ON viewing_requests USING btree (property_id, status); CREATE INDEX IF NOT EXISTS idx_community_memberships_user_id ON community_memberships USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_communities_type ON communities USING btree (type); CREATE INDEX IF NOT EXISTS idx_properties_city_trgm ON properties USING gin (city gin_trgm_ops); CREATE INDEX IF NOT EXISTS idx_properties_featured ON properties USING btree (is_featured, created_at DESC) WHERE is_featured = true; CREATE INDEX IF NOT EXISTS idx_amenities_sort_order ON amenities USING btree (sort_order) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_fairrent_scores_model_version ON fairrent_scores USING btree (model_version); CREATE INDEX IF NOT EXISTS idx_payment_transactions_payer_id ON payment_transactions USING btree (payer_id); CREATE INDEX IF NOT EXISTS idx_verification_documents_user_id ON user_verification_documents USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_monitoring_metrics_name ON monitoring_metrics USING btree (metric_name); CREATE INDEX IF NOT EXISTS idx_profiles_current_country ON profiles USING btree (current_country); CREATE INDEX IF NOT EXISTS idx_profiles_privacy_settings ON profiles USING gin (privacy_settings); CREATE INDEX IF NOT EXISTS idx_deposit_insurance_user_id ON deposit_insurance USING btree (user_id); CREATE INDEX idx_fairrent_scores_letter_grade ON fairrent_scores USING btree (letter_grade); CREATE INDEX IF NOT EXISTS idx_rooms_property_id ON rooms USING btree (property_id); CREATE INDEX IF NOT EXISTS idx_platform_settings_key ON platform_settings USING btree (setting_key); CREATE INDEX IF NOT EXISTS idx_rooms_fairrent_grade ON rooms USING btree (fairrent_letter_grade) WHERE fairrent_score IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_roommate_listings_buddy_up ON roommate_listings USING btree (available_for_buddy_up) WHERE available_for_buddy_up = true; CREATE INDEX IF NOT EXISTS idx_community_categories_locale ON public.community_categories USING btree (locale); CREATE INDEX IF NOT EXISTS idx_roommate_listings_budget ON roommate_listings USING btree (budget_min, budget_max); CREATE INDEX IF NOT EXISTS idx_user_actions_created_at ON user_actions USING btree (created_at DESC); CREATE INDEX IF NOT EXISTS idx_ai_chats_model_used ON ai_chats USING btree (model_used); CREATE INDEX IF NOT EXISTS idx_analytics_user_activity_user_id ON analytics_user_activity USING btree (user_id); CREATE INDEX idx_user_compat_scores_stale ON user_compatibility_scores USING btree (recalculate_after); CREATE INDEX IF NOT EXISTS idx_profiles_field_of_study ON profiles USING btree (field_of_study) WHERE field_of_study IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_profile_views_viewed ON profile_views USING btree (viewed_id, viewed_at DESC); CREATE INDEX IF NOT EXISTS idx_rooms_property_available ON rooms USING btree (property_id, status) WHERE status = 'available'; CREATE INDEX IF NOT EXISTS idx_profiles_verification_status ON profiles USING btree (verification_status); CREATE INDEX IF NOT EXISTS idx_platform_settings_category ON platform_settings USING btree (category); CREATE INDEX IF NOT EXISTS idx_monitoring_config_key ON monitoring_config USING btree (config_key); CREATE INDEX IF NOT EXISTS idx_analytics_user_activity_created_at ON analytics_user_activity USING btree (created_at DESC); CREATE INDEX IF NOT EXISTS idx_notifications_category ON notifications USING btree (category, created_at DESC); CREATE INDEX IF NOT EXISTS idx_user_reports_priority ON user_reports USING btree (priority); CREATE INDEX IF NOT EXISTS idx_ai_chat_votes_chat_id ON ai_chat_votes USING btree (chat_id); CREATE INDEX IF NOT EXISTS idx_user_reports_status ON user_reports USING btree (status); CREATE INDEX IF NOT EXISTS idx_property_interactions_user ON property_interactions USING btree (user_id, created_at DESC); CREATE INDEX IF NOT EXISTS idx_property_options_category ON public.property_options USING btree (category); CREATE INDEX IF NOT EXISTS idx_fairrent_room_scores_room_id ON fairrent_room_scores USING btree (room_id); CREATE INDEX IF NOT EXISTS idx_bookings_start_date ON bookings USING btree (start_date); CREATE INDEX IF NOT EXISTS idx_ai_chats_status ON ai_chats USING btree (status); CREATE INDEX IF NOT EXISTS idx_monitoring_metrics_category ON monitoring_metrics USING btree (metric_category); CREATE INDEX IF NOT EXISTS idx_profiles_user_type ON profiles USING btree (user_type); CREATE INDEX IF NOT EXISTS idx_email_templates_locale ON email_templates USING btree (locale) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_onboarding_options_question ON public.onboarding_question_options USING btree (question_key, is_active); CREATE INDEX IF NOT EXISTS idx_conversation_requests_expires ON conversation_requests USING btree (expires_at); CREATE INDEX IF NOT EXISTS idx_chat_models_sort_order ON chat_models USING btree (sort_order) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_messages_sender ON messages USING btree (sender_id); CREATE INDEX IF NOT EXISTS idx_mbti_personality_types_locale ON mbti_personality_types USING btree (locale) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_ai_chat_messages_chat_created ON ai_chat_messages USING btree (chat_id, created_at ASC); CREATE INDEX IF NOT EXISTS idx_community_posts_community_id ON community_posts USING btree (community_id); CREATE INDEX IF NOT EXISTS idx_buddy_connections_created_at ON buddy_connections USING btree (created_at DESC); CREATE INDEX IF NOT EXISTS idx_market_metrics_market_date ON market_metrics USING btree (market_code, date DESC); CREATE INDEX IF NOT EXISTS idx_user_reports_reporter ON user_reports USING btree (reporter_id); CREATE INDEX IF NOT EXISTS idx_community_posts_author_id ON community_posts USING btree (author_id); CREATE INDEX IF NOT EXISTS idx_validation_messages_key ON public.validation_messages USING btree (validation_key, locale); CREATE INDEX idx_user_compat_scores_version ON user_compatibility_scores USING btree (calculation_version); CREATE INDEX IF NOT EXISTS idx_ai_models_alias ON ai_models USING btree (model_alias); CREATE INDEX IF NOT EXISTS idx_calendar_events_start_date ON calendar_events USING btree (start_date); CREATE INDEX IF NOT EXISTS idx_roommate_listings_status ON roommate_listings USING btree (status); CREATE INDEX IF NOT EXISTS idx_blocked_users_user_id ON blocked_users USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_user_reviews_rating ON user_reviews USING btree (rating, created_at DESC); CREATE INDEX IF NOT EXISTS idx_communities_property_id ON communities USING btree (property_id); CREATE INDEX IF NOT EXISTS idx_webhook_deliveries_created_at ON webhook_deliveries USING btree (created_at DESC); CREATE INDEX IF NOT EXISTS idx_messages_conversation_created ON messages USING btree (conversation_id, created_at DESC); CREATE INDEX IF NOT EXISTS idx_messaging_preferences_user_id ON messaging_preferences USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_profiles_email ON profiles USING btree (email); CREATE INDEX IF NOT EXISTS idx_properties_slug ON properties USING btree (slug); CREATE INDEX IF NOT EXISTS idx_messages_status ON messages USING btree (status); CREATE INDEX IF NOT EXISTS idx_faq_content_sort ON faq_content USING btree (category, sort_order) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_file_uploads_processing ON file_uploads USING btree (processing_status) WHERE processing_status <> 'completed'; CREATE INDEX IF NOT EXISTS idx_roommate_listings_location_prefs ON roommate_listings USING gin (location_preferences); CREATE INDEX IF NOT EXISTS idx_ai_chats_user_created ON ai_chats USING btree (user_id, created_at DESC); CREATE INDEX IF NOT EXISTS idx_buddy_connections_initiated_by ON buddy_connections USING btree (initiated_by); CREATE INDEX IF NOT EXISTS idx_amenities_category ON amenities USING btree (category) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_fairrent_room_scores_room_expires ON fairrent_room_scores USING btree (room_id, expires_at DESC); CREATE INDEX IF NOT EXISTS idx_analytics_user_activity_event_type ON analytics_user_activity USING btree (event_type); CREATE INDEX IF NOT EXISTS idx_feature_usage_logs_user_feature_date ON feature_usage_logs USING btree (user_id, feature_name, date); CREATE INDEX IF NOT EXISTS idx_profiles_status ON profiles USING btree (status); CREATE INDEX IF NOT EXISTS idx_buddy_connections_status ON buddy_connections USING btree (status); CREATE INDEX IF NOT EXISTS idx_ai_chats_visibility ON ai_chats USING btree (visibility); CREATE INDEX IF NOT EXISTS idx_expense_shares_expense_id ON expense_shares USING btree (expense_id); CREATE INDEX IF NOT EXISTS idx_fairrent_room_scores_letter_grade ON fairrent_room_scores USING btree (letter_grade); CREATE INDEX IF NOT EXISTS idx_analytics_events_session ON analytics_events USING btree (session_id); CREATE INDEX IF NOT EXISTS idx_property_options_locale ON public.property_options USING btree (locale); CREATE INDEX IF NOT EXISTS idx_user_reviews_reviewer ON user_reviews USING btree (reviewer_id); CREATE INDEX IF NOT EXISTS idx_fairrent_room_scores_expires_at ON fairrent_room_scores USING btree (expires_at); CREATE INDEX IF NOT EXISTS idx_report_reasons_locale ON public.report_reasons USING btree (locale); CREATE INDEX IF NOT EXISTS idx_translations_lookup ON translations USING btree (locale, namespace, key); CREATE INDEX IF NOT EXISTS idx_api_usage_logs_created_at ON api_usage_logs USING btree (created_at DESC); CREATE INDEX IF NOT EXISTS idx_profiles_mbti_type ON profiles USING btree (mbti_type); CREATE INDEX IF NOT EXISTS idx_email_templates_name ON email_templates USING btree (name) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_ai_chat_files_user_id ON ai_chat_files USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_property_assignments_manager ON property_assignments USING btree (property_manager_id); CREATE INDEX IF NOT EXISTS idx_messages_unread ON messages USING btree (recipient_id, read_at) WHERE read_at IS NULL; CREATE INDEX IF NOT EXISTS idx_profiles_role ON profiles USING btree (role); CREATE INDEX IF NOT EXISTS idx_buddy_connection_members_invited_by ON buddy_connection_members USING btree (invited_by); CREATE INDEX IF NOT EXISTS idx_notification_queue_type ON notification_queue USING btree (type); CREATE INDEX IF NOT EXISTS idx_analytics_user_activity_session ON analytics_user_activity USING btree (session_id); CREATE INDEX IF NOT EXISTS idx_bookings_user_id ON bookings USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_notifications_unread_category ON notifications USING btree (recipient_id, category, created_at DESC) WHERE read_at IS NULL; CREATE INDEX IF NOT EXISTS idx_properties_fairrent_grade ON properties USING btree (fairrent_letter_grade) WHERE fairrent_score IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_properties_title_trgm ON properties USING gin (title gin_trgm_ops); CREATE INDEX IF NOT EXISTS idx_user_actions_target ON user_actions USING btree (target_id, target_type); CREATE INDEX IF NOT EXISTS idx_error_messages_code ON error_messages USING btree (error_code) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_user_safety_scores_score ON user_safety_scores USING btree (overall_score DESC); CREATE INDEX IF NOT EXISTS idx_mbti_questions_category ON mbti_questions USING btree (category) WHERE is_active = true; CREATE INDEX IF NOT EXISTS idx_general_conversations_participant_2 ON general_conversations USING btree (participant_2_id); CREATE INDEX IF NOT EXISTS idx_properties_created_at ON properties USING btree (created_at); CREATE INDEX IF NOT EXISTS idx_roommate_listings_slug ON roommate_listings USING btree (slug); CREATE INDEX IF NOT EXISTS idx_roommate_listings_country ON roommate_listings USING btree (country); CREATE INDEX IF NOT EXISTS idx_ai_usage_tracking_date ON ai_usage_tracking USING btree (date); CREATE INDEX IF NOT EXISTS idx_calendar_events_booking_id ON calendar_events USING btree (booking_id) WHERE booking_id IS NOT NULL; CREATE INDEX IF NOT EXISTS idx_roommate_listings_active ON roommate_listings USING btree (created_at DESC) WHERE status = 'active'; CREATE INDEX IF NOT EXISTS idx_mbti_question_answers_question_id ON mbti_question_answers USING btree (question_id); CREATE INDEX IF NOT EXISTS idx_lifestyle_options_category ON lifestyle_preference_options USING btree (category) WHERE is_active = true; CREATE POLICY buddy_connection_members_view_if_member ON buddy_connection_members FOR SELECT TO public USING (user_id = clerk_user_id() OR clerk_is_admin() OR EXISTS (SELECT 1 FROM buddy_connections bc WHERE bc.id = buddy_connection_id AND bc.initiated_by = clerk_user_id()) OR EXISTS (SELECT 1 FROM buddy_connection_members bcm2 WHERE bcm2.buddy_connection_id = buddy_connection_id AND bcm2.user_id = clerk_user_id() AND bcm2.status = 'active')) ; CREATE POLICY user_reports_create ON user_reports FOR INSERT TO authenticated WITH CHECK (reporter_id = clerk_user_id()) ; CREATE POLICY universities_public_read ON universities FOR SELECT TO public USING (true) ; CREATE POLICY notification_templates_admin_all ON notification_templates TO public USING (clerk_is_admin()) ; CREATE POLICY universities_admin_manage ON universities TO public USING (clerk_is_admin()) ; CREATE POLICY ai_models_admin_all ON ai_models TO public USING (clerk_is_admin()) ; CREATE POLICY chat_models_admin_all ON chat_models TO public USING (clerk_is_admin()) ; CREATE POLICY "Anyone can view property images" ON storage.objects FOR SELECT TO public USING (bucket_id = 'property-images') ; CREATE POLICY buddy_connection_members_leave ON buddy_connection_members FOR DELETE TO authenticated USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY ai_chats_delete_own ON ai_chats FOR DELETE TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY ai_models_public_read_active ON ai_models FOR SELECT TO public USING (is_active = true) ; CREATE POLICY communities_property_owner ON communities TO public USING ((property_id IS NOT NULL AND EXISTS (SELECT 1 FROM properties WHERE id = property_id AND owner_id = clerk_user_id())) OR clerk_is_admin()) ; CREATE POLICY report_reasons_read_all ON report_reasons FOR SELECT TO public USING (active = true) ; CREATE POLICY bookings_create ON bookings FOR INSERT TO public WITH CHECK (user_id = clerk_user_id()) ; CREATE POLICY platform_settings_public_read ON platform_settings FOR SELECT TO public USING (is_public = true) ; CREATE POLICY ai_config_admin_all ON ai_config TO public USING (clerk_is_admin()) ; CREATE POLICY buddy_connection_members_insert_if_member ON buddy_connection_members FOR INSERT TO public WITH CHECK (invited_by = clerk_user_id() AND (EXISTS (SELECT 1 FROM buddy_connections bc WHERE bc.id = buddy_connection_id AND bc.initiated_by = clerk_user_id()) OR EXISTS (SELECT 1 FROM buddy_connection_members bcm WHERE bcm.buddy_connection_id = buddy_connection_id AND bcm.user_id = clerk_user_id() AND bcm.status = 'active'))) ; CREATE POLICY portfolio_items_property_owner ON property_portfolio_items FOR SELECT TO public USING (EXISTS (SELECT 1 FROM properties p WHERE p.id = property_portfolio_items.property_id AND p.owner_id = clerk_user_id())) ; CREATE POLICY personality_responses_own ON personality_test_responses TO public USING (EXISTS (SELECT 1 FROM user_personality_results upr WHERE upr.id = test_result_id AND upr.user_id = clerk_user_id()) OR clerk_is_admin()) WITH CHECK (EXISTS (SELECT 1 FROM user_personality_results upr WHERE upr.id = test_result_id AND upr.user_id = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY "Authenticated users can update fairrent scores" ON fairrent_scores FOR UPDATE TO public USING (is_authenticated() OR current_user = 'service_role') ; CREATE POLICY profiles_delete_safe ON profiles FOR DELETE TO public USING (clerk_is_admin()) ; CREATE POLICY market_configs_public_read ON market_configs FOR SELECT TO public USING (true) ; CREATE POLICY properties_owner_update ON properties FOR UPDATE TO public USING (owner_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (owner_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY portfolio_items_admin ON property_portfolio_items TO public USING (clerk_is_admin()) WITH CHECK (clerk_is_admin()) ; CREATE POLICY "Property owners can upload images" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'property-images' AND current_clerk_claims() ->> 'sub' IS NOT NULL) ; CREATE POLICY lifestyle_options_admin_all ON lifestyle_preference_options TO public USING (clerk_is_admin()) ; CREATE POLICY ai_votes_management ON ai_chat_votes TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY push_subscriptions_own ON push_subscriptions TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY ai_chats_update_own ON ai_chats FOR UPDATE TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY onboarding_question_options_admin_all ON public.onboarding_question_options TO authenticated USING (public.clerk_is_admin()) ; CREATE POLICY market_metrics_admin_only ON market_metrics TO public USING (clerk_is_admin()) ; CREATE POLICY mbti_question_answers_read_all ON mbti_question_answers FOR SELECT TO public USING (true) ; CREATE POLICY posts_author_update ON community_posts FOR UPDATE TO public USING (author_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY verification_docs_update_own ON user_verification_documents FOR UPDATE TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY expense_categories_read_all ON expense_categories FOR SELECT TO public USING (is_active = true) ; CREATE POLICY "Users can upload avatars" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'avatars' AND current_clerk_claims() ->> 'sub' IS NOT NULL AND (storage.foldername(name))[2] = (current_clerk_claims() ->> 'sub')) ; CREATE POLICY avatar_delete_own ON storage.objects FOR DELETE TO authenticated USING (bucket_id = 'avatars' AND current_clerk_claims() ->> 'sub' IS NOT NULL AND (current_clerk_claims() ->> 'sub') = split_part(name, '/', 1)) ; CREATE POLICY ai_usage_limits_public_read ON ai_usage_limits FOR SELECT TO authenticated USING (true) ; CREATE POLICY student_verifications_own ON student_verifications TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY profiles_select_anon_active ON profiles FOR SELECT TO anon USING (status = 'active') ; CREATE POLICY report_templates_read ON report_templates FOR SELECT TO public USING (is_active = true AND (required_user_type IS NULL OR required_user_type = (SELECT user_type FROM profiles WHERE id = clerk_user_id()) OR clerk_is_admin())) ; CREATE POLICY buddy_connections_user_access ON buddy_connections TO authenticated USING (initiated_by = clerk_user_id() OR clerk_is_admin()) WITH CHECK (initiated_by = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY viewing_requests_participant ON viewing_requests FOR SELECT TO public USING (requester_id = clerk_user_id() OR property_manager_id = clerk_user_id() OR EXISTS (SELECT 1 FROM properties p WHERE p.id = viewing_requests.property_id AND p.owner_id = clerk_user_id())) ; CREATE POLICY messages_participants ON messages FOR SELECT TO public USING (sender_id = clerk_user_id() OR recipient_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY "Service role and authenticated users can update fairrent scores" ON fairrent_scores FOR UPDATE TO public USING (is_authenticated() OR current_user = 'service_role') ; ALTER PUBLICATION supabase_realtime ADD TABLE conversations; CREATE POLICY communities_member_write ON communities TO authenticated USING (creator_id = clerk_user_id() OR id IN (SELECT community_id FROM community_memberships WHERE user_id = clerk_user_id() AND status = 'active') OR clerk_is_admin()) WITH CHECK (creator_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY monitoring_admin_only ON monitoring_metrics TO authenticated USING (clerk_is_admin()) ; CREATE POLICY market_legal_docs_public_read ON market_legal_documents FOR SELECT TO public USING (true) ; CREATE POLICY notification_preferences_own ON notification_preferences TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY portfolio_items_insert ON property_portfolio_items FOR INSERT TO public WITH CHECK (EXISTS (SELECT 1 FROM property_portfolios pp WHERE pp.id = portfolio_id AND pp.owner_id = clerk_user_id())) ; CREATE POLICY conversations_participants_only ON conversations TO authenticated USING (participant_1_id = clerk_user_id() OR participant_2_id = clerk_user_id() OR clerk_is_admin() OR user_can_access_org_data(participant_1_id) OR user_can_access_org_data(participant_2_id)) WITH CHECK (participant_1_id = clerk_user_id() OR participant_2_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY onboarding_questions_read_all ON public.onboarding_questions FOR SELECT TO authenticated USING (is_active = true) ; CREATE POLICY translations_admin_manage ON translations TO public USING (clerk_is_admin() OR EXISTS (SELECT 1 FROM profiles WHERE id = clerk_user_id() AND role = 'translator')) ; CREATE POLICY messages_update_recipient ON messages FOR UPDATE TO authenticated USING (recipient_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY validation_messages_admin_all ON public.validation_messages TO authenticated USING (public.clerk_is_admin()) ; CREATE POLICY avatar_upload_own ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'avatars' AND current_clerk_claims() ->> 'sub' IS NOT NULL AND (current_clerk_claims() ->> 'sub') = split_part(name, '/', 1)) ; CREATE POLICY monitoring_config_admin_only ON monitoring_config TO authenticated USING (clerk_is_admin()) ; CREATE POLICY admin_security_settings_admin_only ON admin_security_settings TO public USING (clerk_is_admin()) ; CREATE POLICY ai_chat_files_delete_own ON ai_chat_files FOR DELETE TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY mbti_personality_types_read_all ON mbti_personality_types FOR SELECT TO public USING (is_active = true) ; CREATE POLICY storage_objects_delete_own ON storage.objects FOR DELETE TO authenticated USING (clerk_user_id() IS NOT NULL AND (clerk_user_id() = split_part(name, '/', 1) OR current_clerk_org_id() = split_part(name, '/', 1) OR clerk_is_admin())) ; CREATE POLICY platform_settings_admin_full ON platform_settings TO authenticated USING (clerk_is_admin()) WITH CHECK (clerk_is_admin()) ; CREATE POLICY screening_responses_owner ON screening_responses FOR SELECT TO public USING (EXISTS (SELECT 1 FROM screening_questionnaires sq JOIN properties p ON sq.property_id = p.id WHERE sq.id = questionnaire_id AND p.owner_id = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY payment_transactions_involved_access ON payment_transactions FOR SELECT TO public USING (payer_id = clerk_user_id() OR recipient_id = clerk_user_id() OR EXISTS (SELECT 1 FROM properties WHERE id = property_id AND owner_id = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY bookings_user_access ON bookings FOR SELECT TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY enterprise_relocations_managers ON enterprise_relocations TO public USING (clerk_is_admin() OR clerk_user_id() IN (created_by_id, approved_by_id, hr_contact_id)) ; CREATE POLICY viewing_slots_public ON viewing_availability_slots FOR SELECT TO public USING (is_available = true AND slot_date >= current_date AND EXISTS (SELECT 1 FROM properties p WHERE p.id = viewing_availability_slots.property_id AND p.is_active = true AND p.status = 'available')) ; CREATE POLICY verification_docs_own ON user_verification_documents FOR SELECT TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY user_reviews_update ON user_reviews FOR UPDATE TO public USING (reviewer_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY expense_splits_owner_create ON expense_splits FOR INSERT TO public WITH CHECK (EXISTS (SELECT 1 FROM properties WHERE id = property_id AND owner_id = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY conversation_requests_create ON conversation_requests FOR INSERT TO authenticated WITH CHECK (sender_id = clerk_user_id()) ; CREATE POLICY country_rules_public_read ON country_business_rules FOR SELECT TO public USING (true) ; CREATE POLICY persona_options_read_all ON persona_options FOR SELECT TO public USING (is_active = true) ; CREATE POLICY maintenance_requests_tenant_create ON maintenance_requests FOR INSERT TO public WITH CHECK (tenant_id = clerk_user_id() OR EXISTS (SELECT 1 FROM properties p WHERE p.id = property_id AND p.owner_id = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY messages_delete_own ON messages FOR DELETE TO authenticated USING (sender_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY ai_chat_messages_view_accessible_chats ON ai_chat_messages FOR SELECT TO public USING (EXISTS (SELECT 1 FROM ai_chats WHERE ai_chats.id = ai_chat_messages.chat_id AND (ai_chats.user_id = clerk_user_id() OR ai_chats.visibility = 'public' OR clerk_is_admin()))) ; CREATE POLICY subscription_plans_admin_all ON subscription_plans TO public USING (clerk_is_admin()) ; CREATE POLICY mbti_questions_read_all ON mbti_questions FOR SELECT TO public USING (is_active = true) ; CREATE POLICY expense_splits_creator_update ON expense_splits FOR UPDATE TO public USING (creator_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY buddy_connection_members_invite ON buddy_connection_members FOR INSERT TO authenticated WITH CHECK (invited_by = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY user_subscriptions_own ON user_subscriptions TO public USING (user_id = clerk_user_id()) ; CREATE POLICY user_badges_read ON user_badges FOR SELECT TO public USING (is_visible = true OR user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY buddy_connections_view_if_member ON buddy_connections FOR SELECT TO public USING (initiated_by = clerk_user_id() OR clerk_is_admin() OR EXISTS (SELECT 1 FROM buddy_connection_members bcm WHERE bcm.buddy_connection_id = buddy_connections.id AND bcm.user_id = clerk_user_id() AND bcm.status = 'active')) ; CREATE POLICY viewing_requests_user_or_owner ON viewing_requests TO authenticated USING (requester_id = clerk_user_id() OR property_id IN (SELECT id FROM properties WHERE owner_id = clerk_user_id()) OR clerk_is_admin()) WITH CHECK (requester_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY community_memberships_user_and_admin ON community_memberships TO authenticated USING (user_id = clerk_user_id() OR community_id IN (SELECT id FROM communities WHERE creator_id = clerk_user_id()) OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY screening_questionnaires_owner ON screening_questionnaires TO public USING (EXISTS (SELECT 1 FROM properties WHERE id = property_id AND owner_id = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY communities_creator_manage ON communities TO public USING (creator_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (creator_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY portfolio_items_portfolio_owner ON property_portfolio_items FOR SELECT TO public USING (EXISTS (SELECT 1 FROM property_portfolios pp WHERE pp.id = property_portfolio_items.portfolio_id AND pp.owner_id = clerk_user_id())) ; CREATE POLICY buddy_connection_members_update_own ON buddy_connection_members FOR UPDATE TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY persona_options_admin_all ON persona_options TO public USING (clerk_is_admin()) ; CREATE POLICY neighborhoods_admin_manage ON neighborhoods TO public USING (clerk_is_admin()) WITH CHECK (clerk_is_admin()) ; CREATE POLICY calendar_events_own ON calendar_events TO authenticated USING (user_id = clerk_user_id()) WITH CHECK (user_id = clerk_user_id()) ; CREATE POLICY screening_responses_own ON screening_responses FOR SELECT TO public USING (respondent_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY file_uploads_own ON file_uploads TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY messages_insert_sender ON messages FOR INSERT TO authenticated WITH CHECK (sender_id = clerk_user_id()) ; CREATE POLICY mbti_questions_admin_all ON mbti_questions TO public USING (clerk_is_admin()) ; CREATE POLICY posts_member_create ON community_posts FOR INSERT TO public WITH CHECK (author_id = clerk_user_id() AND community_id IN (SELECT community_id FROM community_memberships WHERE user_id = clerk_user_id() AND status = 'active')) ; CREATE POLICY buddy_connection_members_manage ON buddy_connection_members TO authenticated USING (user_id = clerk_user_id() OR EXISTS (SELECT 1 FROM buddy_connections WHERE id = buddy_connection_id AND initiated_by = clerk_user_id())) WITH CHECK (user_id = clerk_user_id() OR EXISTS (SELECT 1 FROM buddy_connections WHERE id = buddy_connection_id AND initiated_by = clerk_user_id())) ; CREATE POLICY portfolio_items_update ON property_portfolio_items FOR UPDATE TO public USING (EXISTS (SELECT 1 FROM property_portfolios pp WHERE pp.id = property_portfolio_items.portfolio_id AND pp.owner_id = clerk_user_id())) WITH CHECK (EXISTS (SELECT 1 FROM property_portfolios pp WHERE pp.id = portfolio_id AND pp.owner_id = clerk_user_id())) ; CREATE POLICY ai_chat_files_bucket_access ON storage.buckets FOR SELECT TO authenticated USING (id = 'ai-chat-files' AND clerk_user_id() IS NOT NULL) ; CREATE POLICY security_deposits_owner_access ON security_deposits FOR SELECT TO public USING (EXISTS (SELECT 1 FROM properties WHERE id = property_id AND owner_id = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY profiles_public_read ON profiles FOR SELECT TO public USING (true) ; CREATE POLICY university_domains_public_read ON university_email_domains FOR SELECT TO public USING (true) ; CREATE POLICY conversations_update_own ON conversations FOR UPDATE TO authenticated USING (participant_1_id = clerk_user_id() OR participant_2_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY ai_chats_insert_own ON ai_chats FOR INSERT TO public WITH CHECK (user_id = clerk_user_id()) ; CREATE POLICY ai_routing_rules_admin_all ON ai_routing_rules TO public USING (clerk_is_admin()) ; CREATE POLICY bookings_owner_access ON bookings FOR SELECT TO public USING (EXISTS (SELECT 1 FROM rooms r JOIN properties p ON r.property_id = p.id WHERE r.id = room_id AND p.owner_id = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY "Users can view their own compatibility scores" ON user_compatibility_scores FOR SELECT TO authenticated USING (user1_id = clerk_user_id() OR user2_id = clerk_user_id()) ; CREATE POLICY screening_responses_create ON screening_responses FOR INSERT TO public WITH CHECK (respondent_id = clerk_user_id()) ; CREATE POLICY country_content_admin_manage ON country_content TO public USING (clerk_is_admin()) ; CREATE POLICY api_usage_logs_own ON api_usage_logs FOR SELECT TO public USING (organization_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY events_member_create ON community_events FOR INSERT TO public WITH CHECK (organizer_id = clerk_user_id() AND community_id IN (SELECT community_id FROM community_memberships WHERE user_id = clerk_user_id() AND status = 'active')) ; CREATE POLICY faq_content_admin_all ON faq_content TO public USING (clerk_is_admin()) ; CREATE POLICY report_requests_own ON report_requests TO authenticated USING (user_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id()) ; CREATE POLICY avatar_update_own ON storage.objects FOR UPDATE TO authenticated USING (bucket_id = 'avatars' AND current_clerk_claims() ->> 'sub' IS NOT NULL AND (current_clerk_claims() ->> 'sub') = split_part(name, '/', 1)) ; CREATE POLICY ai_chat_messages_insert_own_chats ON ai_chat_messages FOR INSERT TO public WITH CHECK (EXISTS (SELECT 1 FROM ai_chats WHERE ai_chats.id = ai_chat_messages.chat_id AND (ai_chats.user_id = clerk_user_id() OR clerk_is_admin()))) ; CREATE POLICY "Users can upload documents" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'verification-documents' AND (storage.foldername(name))[2] = (current_clerk_claims() ->> 'sub')) ; CREATE POLICY email_templates_read_all ON email_templates FOR SELECT TO public USING (is_active = true) ; CREATE POLICY "Service role and authenticated users can update room fairrent s" ON fairrent_room_scores FOR UPDATE TO public USING (is_authenticated() OR current_user = 'service_role') ; CREATE POLICY storage_objects_org_shared ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'org-shared' AND current_clerk_org_id() = split_part(name, '/', 1)) ; CREATE POLICY security_deposits_tenant_access ON security_deposits FOR SELECT TO public USING (tenant_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY "Service role can manage compatibility scores" ON user_compatibility_scores TO service_role USING (true) WITH CHECK (true) ; CREATE POLICY events_organizer_update ON community_events FOR UPDATE TO public USING (organizer_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY "Authenticated users can insert fairrent scores" ON fairrent_scores FOR INSERT TO public WITH CHECK (is_authenticated() OR current_user = 'service_role') ; CREATE POLICY properties_owner_delete ON properties FOR DELETE TO public USING (owner_id = clerk_user_id() OR clerk_is_admin()) ; DROP POLICY IF EXISTS roommate_listings_public_view ON roommate_listings; CREATE POLICY ai_routing_rules_public_read_active ON ai_routing_rules FOR SELECT TO public USING (is_active = true) ; CREATE POLICY ai_chat_votes_manage_own ON ai_chat_votes TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id()) ; CREATE POLICY ai_chat_messages_update_own_chats ON ai_chat_messages FOR UPDATE TO public USING (EXISTS (SELECT 1 FROM ai_chats WHERE ai_chats.id = ai_chat_messages.chat_id AND (ai_chats.user_id = clerk_user_id() OR clerk_is_admin()))) ; CREATE POLICY translations_public_read ON translations FOR SELECT TO public USING (true) ; CREATE POLICY property_interests_user_insert ON property_interests FOR INSERT TO authenticated WITH CHECK (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY mass_message_campaigns_own ON mass_message_campaigns TO public USING (created_by = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY ai_files_management ON ai_chat_files TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY profiles_insert_safe ON profiles FOR INSERT TO public WITH CHECK (clerk_user_id() = id OR clerk_is_admin()) ; CREATE POLICY properties_public_read ON properties FOR SELECT TO public USING ((is_active = true AND status = 'available') OR owner_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY deposit_insurance_user_access ON deposit_insurance TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY portfolio_items_delete ON property_portfolio_items FOR DELETE TO public USING (EXISTS (SELECT 1 FROM property_portfolios pp WHERE pp.id = property_portfolio_items.portfolio_id AND pp.owner_id = clerk_user_id())) ; CREATE POLICY roommate_listings_own_manage ON roommate_listings TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY notification_queue_system_insert ON notification_queue FOR INSERT TO authenticated WITH CHECK (true) ; CREATE POLICY market_configs_admin_manage ON market_configs TO public USING (clerk_is_admin()) ; CREATE POLICY maintenance_requests_stakeholder_read ON maintenance_requests FOR SELECT TO public USING (EXISTS (SELECT 1 FROM properties p WHERE p.id = property_id AND p.owner_id = clerk_user_id()) OR tenant_id = clerk_user_id() OR assigned_to = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY user_reports_admin_manage ON user_reports FOR UPDATE TO authenticated USING (clerk_is_admin()) ; CREATE POLICY enterprise_relocations_employee ON enterprise_relocations FOR SELECT TO public USING (employee_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY notifications_recipient ON notifications FOR SELECT TO public USING (recipient_id = clerk_user_id()) ; CREATE POLICY posts_moderator_manage ON community_posts TO public USING (community_id IN (SELECT community_id FROM community_memberships WHERE user_id = clerk_user_id() AND role IN ('admin', 'moderator', 'owner')) OR clerk_is_admin()) ; CREATE POLICY ui_text_content_admin_all ON public.ui_text_content TO authenticated USING (public.clerk_is_admin()) ; CREATE POLICY user_safety_scores_own ON user_safety_scores FOR SELECT TO public USING (user_id = clerk_user_id()) ; CREATE POLICY ai_chat_files_view_own_or_accessible ON ai_chat_files FOR SELECT TO public USING (user_id = clerk_user_id() OR clerk_is_admin() OR (chat_id IS NOT NULL AND EXISTS (SELECT 1 FROM ai_chats WHERE ai_chats.id = ai_chat_files.chat_id AND ai_chats.visibility = 'public'))) ; CREATE POLICY onboarding_steps_admin_all ON public.onboarding_steps TO authenticated USING (public.clerk_is_admin()) ; CREATE POLICY notifications_recipient_access ON notifications TO public USING (recipient_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (recipient_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY validation_messages_read_all ON public.validation_messages FOR SELECT TO authenticated USING (is_active = true) ; CREATE POLICY viewing_requests_create ON viewing_requests FOR INSERT TO public WITH CHECK (requester_id = clerk_user_id()) ; CREATE POLICY community_categories_read_all ON community_categories FOR SELECT TO public USING (active = true) ; CREATE POLICY ai_chat_messages_delete_own_chats ON ai_chat_messages FOR DELETE TO public USING (EXISTS (SELECT 1 FROM ai_chats WHERE ai_chats.id = ai_chat_messages.chat_id AND (ai_chats.user_id = clerk_user_id() OR clerk_is_admin()))) ; CREATE POLICY memberships_admin_manage ON community_memberships TO public USING (community_id IN (SELECT community_id FROM community_memberships WHERE user_id = clerk_user_id() AND role IN ('admin', 'owner')) OR clerk_is_admin()) ; CREATE POLICY ai_chat_files_upload_own_folder ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'ai-chat-files' AND clerk_user_id() IS NOT NULL AND (storage.foldername(name))[1] = clerk_user_id()) ; CREATE POLICY notification_templates_read_all ON notification_templates FOR SELECT TO public USING (is_active = true) ; CREATE POLICY buddy_connections_update_if_member ON buddy_connections FOR UPDATE TO public USING (initiated_by = clerk_user_id() OR clerk_is_admin() OR EXISTS (SELECT 1 FROM buddy_connection_members bcm WHERE bcm.buddy_connection_id = buddy_connections.id AND bcm.user_id = clerk_user_id() AND bcm.status = 'active')) ; CREATE POLICY report_reasons_admin_all ON report_reasons TO public USING (clerk_is_admin()) ; CREATE POLICY storage_objects_select_own ON storage.objects FOR SELECT TO authenticated USING (clerk_user_id() IS NOT NULL AND (clerk_user_id() = split_part(name, '/', 1) OR current_clerk_org_id() = split_part(name, '/', 1) OR clerk_is_admin())) ; CREATE POLICY ai_chats_access ON ai_chats FOR SELECT TO public USING (user_id = clerk_user_id() OR visibility = 'public' OR clerk_is_admin()) ; CREATE POLICY "Users can view own documents" ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'verification-documents' AND (storage.foldername(name))[2] = (current_clerk_claims() ->> 'sub')) ; CREATE POLICY user_badges_admin_manage ON user_badges TO public USING (clerk_is_admin()) WITH CHECK (clerk_is_admin()) ; CREATE POLICY country_content_public_read ON country_content FOR SELECT TO public USING (true) ; CREATE POLICY screening_results_own ON tenant_screening_results FOR SELECT TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY user_reviews_read ON user_reviews FOR SELECT TO public USING (is_public = true OR reviewee_id = clerk_user_id() OR reviewer_id = clerk_user_id()) ; CREATE POLICY property_managers_property_owner ON property_managers TO public USING (EXISTS (SELECT 1 FROM properties p WHERE p.id = property_managers.property_id AND p.owner_id = clerk_user_id()) OR user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY communities_public_read ON communities FOR SELECT TO public USING (is_private = false OR clerk_is_admin()) ; CREATE POLICY properties_select_public ON properties FOR SELECT TO public USING ((status = 'available' AND is_active = true) OR (auth.role() = 'authenticated' AND owner_id = clerk_user_id()) OR (auth.role() = 'authenticated' AND clerk_is_admin())) ; CREATE POLICY community_posts_member_write ON community_posts TO authenticated USING (author_id = clerk_user_id() OR community_id IN (SELECT community_id FROM community_memberships WHERE user_id = clerk_user_id() AND status = 'active') OR clerk_is_admin()) WITH CHECK (author_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY messaging_preferences_own ON messaging_preferences TO authenticated USING (user_id = clerk_user_id()) WITH CHECK (user_id = clerk_user_id()) ; CREATE POLICY bookings_user_or_owner ON bookings TO authenticated USING (user_id = clerk_user_id() OR room_id IN (SELECT r.id FROM rooms r JOIN properties p ON r.property_id = p.id WHERE p.owner_id = clerk_user_id()) OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY "Anyone can view fairrent scores" ON fairrent_scores FOR SELECT TO public USING (true) ; CREATE POLICY saved_searches_own ON saved_searches TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY property_multimedia_public_read ON property_multimedia FOR SELECT TO public USING (EXISTS (SELECT 1 FROM properties p WHERE p.id = property_id AND p.is_active = true) OR EXISTS (SELECT 1 FROM properties p WHERE p.id = property_id AND p.owner_id = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY financial_forecasts_owner_all ON financial_forecasts TO public USING (portfolio_owner_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY messages_conversation_participants ON messages FOR SELECT TO authenticated USING (sender_id = clerk_user_id() OR recipient_id = clerk_user_id() OR clerk_is_admin() OR user_can_access_org_data(sender_id) OR user_can_access_org_data(recipient_id)) ; CREATE POLICY properties_manage_own ON properties TO public USING (owner_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (owner_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY market_legal_docs_admin_manage ON market_legal_documents TO public USING (clerk_is_admin()) ; CREATE POLICY messages_update_own ON messages FOR UPDATE TO authenticated USING (sender_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY mbti_question_answers_admin_all ON mbti_question_answers TO public USING (clerk_is_admin()) ; CREATE POLICY "Service role and authenticated users can insert room fairrent s" ON fairrent_room_scores FOR INSERT TO public WITH CHECK (is_authenticated() OR current_user = 'service_role') ; CREATE POLICY "Anyone can view avatars" ON storage.objects FOR SELECT TO public USING (bucket_id = 'avatars') ; CREATE POLICY payment_transactions_user_only ON payment_transactions TO authenticated USING (payer_id = clerk_user_id() OR recipient_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (payer_id = clerk_user_id() OR recipient_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY conversations_participants ON conversations TO public USING (participant_1_id = clerk_user_id() OR participant_2_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY posts_member_access ON community_posts FOR SELECT TO public USING (community_id IN (SELECT community_id FROM community_memberships WHERE user_id = clerk_user_id() AND status = 'active') OR clerk_is_admin()) ; CREATE POLICY enterprise_relocations_create ON enterprise_relocations FOR INSERT TO public WITH CHECK (employee_id = clerk_user_id()) ; CREATE POLICY maintenance_requests_stakeholder_update ON maintenance_requests FOR UPDATE TO public USING (EXISTS (SELECT 1 FROM properties p WHERE p.id = property_id AND p.owner_id = clerk_user_id()) OR assigned_to = clerk_user_id() OR clerk_is_admin()) WITH CHECK (EXISTS (SELECT 1 FROM properties p WHERE p.id = property_id AND p.owner_id = clerk_user_id()) OR assigned_to = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY ai_messages_access ON ai_chat_messages FOR SELECT TO public USING (EXISTS (SELECT 1 FROM ai_chats WHERE ai_chats.id = ai_chat_messages.chat_id AND (ai_chats.user_id = clerk_user_id() OR ai_chats.visibility = 'public' OR clerk_is_admin()))) ; CREATE POLICY chat_models_read_all ON chat_models FOR SELECT TO public USING (is_active = true) ; CREATE POLICY relocation_requests_own ON user_relocation_requests TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY messages_insert_own ON messages FOR INSERT TO authenticated WITH CHECK (sender_id = clerk_user_id() AND conversation_id IN (SELECT id FROM conversations WHERE participant_1_id = clerk_user_id() OR participant_2_id = clerk_user_id())) ; CREATE POLICY buddy_connections_member_read ON buddy_connections FOR SELECT TO authenticated USING (initiated_by = clerk_user_id() OR EXISTS (SELECT 1 FROM buddy_connection_members WHERE buddy_connection_id = id AND user_id = clerk_user_id() AND status = 'active')) ; CREATE POLICY "Service role and authenticated users can insert fairrent scores" ON fairrent_scores FOR INSERT TO public WITH CHECK (is_authenticated() OR current_user = 'service_role') ; CREATE POLICY ui_text_content_read_all ON public.ui_text_content FOR SELECT TO authenticated USING (is_active = true) ; CREATE POLICY analytics_events_own_data ON analytics_events TO authenticated USING (user_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id()) ; CREATE POLICY ai_chats_management ON ai_chats TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY user_notifications_own ON user_notifications TO authenticated USING (user_id = clerk_user_id()) WITH CHECK (user_id = clerk_user_id()) ; CREATE POLICY profiles_select_safe ON profiles FOR SELECT TO authenticated USING (clerk_user_id() = id OR status = 'active' OR clerk_is_admin()) ; CREATE POLICY saved_properties_own ON saved_properties TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY conversation_requests_respond ON conversation_requests FOR UPDATE TO authenticated USING (recipient_id = clerk_user_id()) ; CREATE POLICY profiles_update_safe ON profiles FOR UPDATE TO public USING (clerk_user_id() = id OR clerk_is_admin()) WITH CHECK (clerk_user_id() = id OR clerk_is_admin()) ; CREATE POLICY enterprise_webhooks_own ON enterprise_webhooks TO public USING (organization_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY countries_admin_all ON countries TO public USING (clerk_is_admin()) ; CREATE POLICY university_partnerships_admin_manage ON university_partnerships TO public USING (clerk_is_admin()) ; CREATE POLICY notifications_update_own ON notifications FOR UPDATE TO public USING (recipient_id = clerk_user_id()) WITH CHECK (recipient_id = clerk_user_id()) ; CREATE POLICY interactions_member_access ON community_post_interactions TO public USING ((user_id = clerk_user_id() AND post_id IN (SELECT cp.id FROM community_posts cp JOIN community_memberships cm ON cp.community_id = cm.community_id WHERE cm.user_id = clerk_user_id() AND cm.status = 'active')) OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id() AND post_id IN (SELECT cp.id FROM community_posts cp JOIN community_memberships cm ON cp.community_id = cm.community_id WHERE cm.user_id = clerk_user_id() AND cm.status = 'active')) ; CREATE POLICY verification_flows_own ON verification_flows TO authenticated USING (user_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id()) ; CREATE POLICY countries_read_all ON countries FOR SELECT TO public USING (is_active = true) ; CREATE POLICY onboarding_steps_read_all ON public.onboarding_steps FOR SELECT TO authenticated USING (is_active = true) ; CREATE POLICY ai_chat_files_delete_own_folder ON storage.objects FOR DELETE TO authenticated USING (bucket_id = 'ai-chat-files' AND clerk_user_id() IS NOT NULL AND (storage.foldername(name))[1] = clerk_user_id()) ; CREATE POLICY events_member_access ON community_events FOR SELECT TO public USING (community_id IN (SELECT community_id FROM community_memberships WHERE user_id = clerk_user_id() AND status = 'active') OR clerk_is_admin()) ; CREATE POLICY buddy_connection_members_delete_if_admin_or_initiator ON buddy_connection_members FOR DELETE TO public USING (clerk_is_admin() OR EXISTS (SELECT 1 FROM buddy_connections bc WHERE bc.id = buddy_connection_id AND bc.initiated_by = clerk_user_id()) OR user_id = clerk_user_id()) ; CREATE POLICY ai_chat_files_view_own_folder ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'ai-chat-files' AND clerk_user_id() IS NOT NULL AND (storage.foldername(name))[1] = clerk_user_id()) ; CREATE POLICY user_reports_reporter ON user_reports FOR SELECT TO authenticated USING (reporter_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY buddy_connections_insert_own ON buddy_connections FOR INSERT TO public WITH CHECK (initiated_by = clerk_user_id()) ; CREATE POLICY community_categories_admin_all ON community_categories TO public USING (clerk_is_admin()) ; CREATE POLICY property_interests_user_or_manager ON property_interests FOR SELECT TO authenticated USING (user_id = clerk_user_id() OR property_id IN (SELECT id FROM properties WHERE owner_id = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY email_templates_admin_all ON email_templates TO public USING (clerk_is_admin()) ; CREATE POLICY subscription_plans_read_all ON subscription_plans FOR SELECT TO public USING (is_active = true) ; CREATE POLICY ai_chat_files_insert_own ON ai_chat_files FOR INSERT TO public WITH CHECK (user_id = clerk_user_id()) ; CREATE POLICY storage_objects_insert_own ON storage.objects FOR INSERT TO authenticated WITH CHECK (clerk_user_id() IS NOT NULL AND (clerk_user_id() = split_part(name, '/', 1) OR current_clerk_org_id() = split_part(name, '/', 1) OR clerk_is_admin())) ; CREATE POLICY property_analytics_owner_read ON property_analytics FOR SELECT TO public USING (EXISTS (SELECT 1 FROM properties p WHERE p.id = property_id AND p.owner_id = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY bookings_owner_update ON bookings FOR UPDATE TO public USING (EXISTS (SELECT 1 FROM rooms r JOIN properties p ON r.property_id = p.id WHERE r.id = room_id AND p.owner_id = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY amenities_read_all ON amenities FOR SELECT TO public USING (is_active = true) ; CREATE POLICY relocation_services_admin_manage ON relocation_services TO public USING (clerk_is_admin()) ; CREATE POLICY currency_rates_public_read ON currency_rates FOR SELECT TO public USING (true) ; CREATE POLICY neighborhoods_public_read ON neighborhoods FOR SELECT TO public USING (true) ; CREATE POLICY user_reviews_write ON user_reviews FOR INSERT TO public WITH CHECK (reviewer_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY user_preferences_own ON user_preferences TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY webhook_deliveries_admin ON webhook_deliveries TO authenticated USING (clerk_is_admin()) ; CREATE POLICY analytics_user_activity_own ON analytics_user_activity TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY onboarding_questions_admin_all ON public.onboarding_questions TO authenticated USING (public.clerk_is_admin()) ; CREATE POLICY matches_user_participant ON matches TO public USING (user1_id = clerk_user_id() OR user2_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (user1_id = clerk_user_id() OR user2_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY roommate_listings_own ON roommate_listings TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY marketing_campaigns_own_all ON marketing_campaigns TO public USING (created_by = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY expense_categories_admin_all ON expense_categories TO public USING (clerk_is_admin()) ; CREATE POLICY lifestyle_options_read_all ON lifestyle_preference_options FOR SELECT TO public USING (is_active = true) ; CREATE POLICY blocked_users_own ON blocked_users TO authenticated USING (user_id = clerk_user_id()) WITH CHECK (user_id = clerk_user_id()) ; CREATE POLICY amenities_admin_all ON amenities TO public USING (clerk_is_admin()) ; CREATE POLICY email_templates_admin_manage ON email_templates TO public USING (clerk_is_admin()) ; CREATE POLICY community_posts_public_read ON community_posts FOR SELECT TO public USING (community_id IN (SELECT id FROM communities WHERE is_private = false) OR clerk_is_admin()) ; CREATE POLICY expense_shares_own_access ON expense_shares FOR SELECT TO public USING (user_id = clerk_user_id() OR EXISTS (SELECT 1 FROM expense_splits es JOIN properties p ON es.property_id = p.id WHERE es.id = expense_id AND p.owner_id = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY conversations_create ON conversations FOR INSERT TO authenticated WITH CHECK ((participant_1_id = clerk_user_id() OR participant_2_id = clerk_user_id()) AND participant_1_id <> participant_2_id) ; CREATE POLICY properties_owner_write ON properties FOR INSERT TO public WITH CHECK (owner_id = clerk_user_id()) ; CREATE POLICY memberships_join ON community_memberships FOR INSERT TO public WITH CHECK (user_id = clerk_user_id()) ; CREATE POLICY conversation_requests_involved ON conversation_requests FOR SELECT TO authenticated USING (sender_id = clerk_user_id() OR recipient_id = clerk_user_id()) ; CREATE POLICY mbti_personality_types_admin_all ON mbti_personality_types TO public USING (clerk_is_admin()) ; CREATE POLICY expense_splits_owner_access ON expense_splits FOR SELECT TO public USING (EXISTS (SELECT 1 FROM properties WHERE id = property_id AND owner_id = clerk_user_id()) OR creator_id = clerk_user_id() OR EXISTS (SELECT 1 FROM expense_shares WHERE expense_id = id AND user_id = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY currency_rates_admin_write ON currency_rates TO authenticated USING (clerk_is_admin()) ; CREATE POLICY screening_results_owner ON tenant_screening_results FOR SELECT TO public USING (EXISTS (SELECT 1 FROM properties WHERE id = property_id AND owner_id = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY error_messages_read_all ON error_messages FOR SELECT TO public USING (is_active = true) ; CREATE POLICY report_templates_admin_all ON report_templates TO public USING (clerk_is_admin()) ; CREATE POLICY faq_content_read_all ON faq_content FOR SELECT TO public USING (is_active = true) ; CREATE POLICY property_options_admin_all ON property_options TO public USING (clerk_is_admin()) ; CREATE POLICY bookings_update_own ON bookings FOR UPDATE TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY buddy_connection_members_involved ON buddy_connection_members FOR SELECT TO authenticated USING (user_id = clerk_user_id() OR EXISTS (SELECT 1 FROM buddy_connections WHERE id = buddy_connection_id AND initiated_by = clerk_user_id())) ; CREATE POLICY ai_chat_files_update_own ON ai_chat_files FOR UPDATE TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY conversations_participants_view ON conversations FOR SELECT TO authenticated USING (participant_1_id = clerk_user_id() OR participant_2_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY university_partnerships_public_read ON university_partnerships FOR SELECT TO public USING (true) ; CREATE POLICY buddy_connection_members_participants ON buddy_connection_members FOR SELECT TO authenticated USING (invited_by = clerk_user_id() OR user_id = clerk_user_id() OR EXISTS (SELECT 1 FROM buddy_connections bc WHERE bc.id = buddy_connection_id AND bc.initiated_by = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY match_metrics_participant ON match_metrics TO authenticated USING (user_id = clerk_user_id() OR EXISTS (SELECT 1 FROM matches m WHERE m.id = match_id AND (m.user1_id = clerk_user_id() OR m.user2_id = clerk_user_id())) OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id()) ; CREATE POLICY mbti_assessments_own ON mbti_assessments TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY ai_usage_tracking_view_own ON ai_usage_tracking FOR SELECT TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY memberships_member_access ON community_memberships FOR SELECT TO public USING (user_id = clerk_user_id() OR community_id IN (SELECT community_id FROM community_memberships WHERE user_id = clerk_user_id() AND role IN ('admin', 'moderator', 'owner')) OR clerk_is_admin()) ; CREATE POLICY ai_chats_view_own_or_public ON ai_chats FOR SELECT TO public USING (user_id = clerk_user_id() OR visibility = 'public' OR clerk_is_admin()) ; CREATE POLICY buddy_connections_creator_manage ON buddy_connections TO authenticated USING (initiated_by = clerk_user_id()) WITH CHECK (initiated_by = clerk_user_id()) ; CREATE POLICY user_actions_own ON user_actions TO authenticated USING (user_id = clerk_user_id()) WITH CHECK (user_id = clerk_user_id()) ; CREATE POLICY relocation_services_public_read ON relocation_services FOR SELECT TO public USING (true) ; CREATE POLICY messages_view_participants ON messages FOR SELECT TO authenticated USING (sender_id = clerk_user_id() OR recipient_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY profiles_own_write ON profiles TO public USING (id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY personality_results_own ON user_personality_results TO public USING (user_id = clerk_user_id() OR clerk_is_admin()) WITH CHECK (user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY onboarding_question_options_read_all ON public.onboarding_question_options FOR SELECT TO authenticated USING (is_active = true) ; CREATE POLICY general_conversations_participants ON general_conversations TO authenticated USING (participant_1_id = clerk_user_id() OR participant_2_id = clerk_user_id()) WITH CHECK (participant_1_id = clerk_user_id() OR participant_2_id = clerk_user_id()) ; CREATE POLICY university_domains_admin_manage ON university_email_domains TO public USING (clerk_is_admin()) ; CREATE POLICY verification_docs_upload ON user_verification_documents FOR INSERT TO public WITH CHECK (user_id = clerk_user_id()) ; CREATE POLICY "Anyone can view room fairrent scores" ON fairrent_room_scores FOR SELECT TO public USING (true) ; CREATE POLICY ai_chat_files_update_own_folder ON storage.objects FOR UPDATE TO authenticated USING (bucket_id = 'ai-chat-files' AND clerk_user_id() IS NOT NULL AND (storage.foldername(name))[1] = clerk_user_id()) ; CREATE POLICY error_messages_admin_all ON error_messages TO public USING (clerk_is_admin()) ; CREATE POLICY roommate_listings_public_read ON roommate_listings FOR SELECT TO public USING (status = 'active' OR user_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY notification_queue_own ON notification_queue FOR SELECT TO authenticated USING (user_id = clerk_user_id()) ; CREATE POLICY messages_send ON messages FOR INSERT TO public WITH CHECK (sender_id = clerk_user_id()) ; CREATE POLICY ai_messages_insert ON ai_chat_messages FOR INSERT TO public WITH CHECK (EXISTS (SELECT 1 FROM ai_chats WHERE ai_chats.id = ai_chat_messages.chat_id AND (ai_chats.user_id = clerk_user_id() OR clerk_is_admin()))) ; CREATE POLICY country_rules_admin_manage ON country_business_rules TO public USING (clerk_is_admin()) ; CREATE POLICY ai_usage_limits_admin_all ON ai_usage_limits TO public USING (clerk_is_admin()) ; CREATE POLICY rooms_property_owner_manage ON rooms TO public USING (EXISTS (SELECT 1 FROM properties p WHERE p.id = property_id AND p.owner_id = clerk_user_id()) OR clerk_is_admin()) WITH CHECK (EXISTS (SELECT 1 FROM properties p WHERE p.id = property_id AND p.owner_id = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY ai_usage_tracking_system_manage ON ai_usage_tracking TO public USING (clerk_is_admin()) ; CREATE POLICY ai_chat_votes_view_accessible_chats ON ai_chat_votes FOR SELECT TO public USING (EXISTS (SELECT 1 FROM ai_chats WHERE ai_chats.id = ai_chat_votes.chat_id AND (ai_chats.user_id = clerk_user_id() OR ai_chats.visibility = 'public' OR clerk_is_admin()))) ; CREATE POLICY rooms_public_read ON rooms FOR SELECT TO public USING (EXISTS (SELECT 1 FROM properties p WHERE p.id = property_id AND (p.is_active = true OR p.owner_id = clerk_user_id())) OR clerk_is_admin()) ; CREATE POLICY property_multimedia_owner_manage ON property_multimedia TO public USING (EXISTS (SELECT 1 FROM properties p WHERE p.id = property_id AND p.owner_id = clerk_user_id()) OR clerk_is_admin()) WITH CHECK (EXISTS (SELECT 1 FROM properties p WHERE p.id = property_id AND p.owner_id = clerk_user_id()) OR clerk_is_admin()) ; CREATE POLICY property_assignments_access ON property_assignments TO public USING (property_manager_id = clerk_user_id() OR clerk_is_admin()) ; CREATE POLICY property_options_read_all ON property_options FOR SELECT TO public USING (active = true) ; CREATE POLICY property_analytics_system_write ON property_analytics FOR INSERT TO public WITH CHECK (clerk_is_admin()) ; COMMENT ON TABLE user_compatibility_scores IS 'Precomputed compatibility scores between users with factor breakdowns.
Recalculated every 15 days or on-demand. Single source of truth for all compatibility calculations.'; COMMENT ON TABLE subscription_plans IS 'Available subscription plans with pricing and features'; COMMENT ON COLUMN properties.manager_id IS 'Optional property manager separate from owner'; COMMENT ON TABLE monitoring_metrics IS 'System performance and health monitoring metrics'; COMMENT ON TABLE deposit_insurance IS 'Deposit protection insurance policies'; COMMENT ON TABLE user_actions IS 'User interaction tracking for matching and engagement analytics'; COMMENT ON COLUMN profiles.privacy_settings IS 'User privacy preferences controlling profile visibility and communication. Includes flags for public_profile, show_age, show_email, show_phone, show_location, allow_messages, allow_buddy_up_requests.'; BEGIN; COMMENT ON TABLE ai_chat_votes IS 'User feedback votes on AI chat messages (-1 downvote, 1 upvote)'; COMMENT ON TABLE ai_chat_votes IS 'User feedback votes on AI chat messages'; COMMENT ON TABLE ai_chat_votes IS 'User feedback votes on AI chat messages'; COMMIT; COMMENT ON COLUMN fairrent_scores.urgency IS 'Deal urgency: high (act fast), medium (good deal), low (normal)'; COMMENT ON COLUMN buddy_connections.max_members IS 'Maximum members allowed in this connection (2-8)'; COMMENT ON COLUMN messages.reply_to_id IS 'Reference to message being replied to (threading support)'; COMMENT ON TABLE report_templates IS 'Report templates for generating various business reports'; BEGIN; COMMENT ON COLUMN ai_chats.visibility IS 'Chat visibility: private (owner only) or public (viewable by all)'; COMMENT ON COLUMN ai_chats.visibility IS 'Chat visibility: private (owner only) or public (everyone can view)'; COMMENT ON COLUMN ai_chats.visibility IS 'Chat visibility: private (owner only) or public (everyone can view)'; COMMIT; COMMENT ON COLUMN profiles.allow_cold_messages IS 'Whether user allows messages from non-matches (when in hybrid/open mode)'; COMMENT ON COLUMN fairrent_scores.model_version IS 'ML model version used (e.g., "v2.3.5-extended-autoregressive")'; COMMENT ON TABLE roommate_listings IS 'User listings for finding rooms or roommates'; COMMENT ON TABLE matches IS 'User matches for roommate and buddy connections'; COMMENT ON TABLE user_reports IS 'User safety reporting system with priority levels'; COMMENT ON TABLE general_conversations IS 'Conversations not tied to matches (property inquiries, buddy-up, etc.)'; COMMENT ON COLUMN fairrent_scores.letter_grade IS 'A (85-100), B+ (75-84), B (60-74), C (45-59), D (30-44), F (0-29)'; COMMENT ON TABLE analytics_user_activity IS 'User activity tracking for analytics'; COMMENT ON COLUMN properties.security_deposit IS 'Security deposit amount required for this property. NULL means same as rent or not specified.'; COMMENT ON TABLE analytics_events IS 'Consolidated user activity and performance analytics'; COMMENT ON COLUMN fairrent_scores.expires_at IS 'Score expires after 7 days - recalculation needed'; COMMENT ON TABLE buddy_connection_members IS 'Members of buddy connections with invitation and status tracking'; COMMENT ON TABLE ai_models IS 'Admin-controlled AI model configurations'; COMMENT ON COLUMN buddy_connections.name IS 'Optional group name, null for direct 1:1 connections'; COMMENT ON TABLE enterprise_webhooks IS 'Webhook configurations for enterprise integrations'; COMMENT ON TABLE lifestyle_preference_options IS 'Options for lifestyle preference forms'; COMMENT ON COLUMN profiles.auth_provider IS 'Authentication provider - always "clerk" for this app'; COMMENT ON TABLE countries IS 'Countries reference data for international platform'; COMMENT ON COLUMN profiles.portfolio_size IS 'Current number of properties actively managed by property manager. Distinct from portfolio_size_limit which is the maximum allowed based on subscription tier.'; BEGIN; COMMENT ON TABLE ai_chats IS 'AI chat conversations with subscription-aware access control'; COMMENT ON TABLE ai_chats IS 'AI chat conversations with visibility control'; COMMENT ON TABLE ai_chats IS 'AI chat conversations with visibility control'; COMMIT; COMMENT ON TABLE notification_templates IS 'In-app notification templates'; COMMENT ON POLICY profiles_select_safe ON profiles IS 'Allow authenticated users to view their own profile, active profiles, or all profiles if admin'; COMMENT ON VIEW public_profiles IS 'Public view of active profiles with security_invoker to respect RLS policies'; COMMENT ON COLUMN ai_models.deployment_name IS 'Azure-specific deployment name'; COMMENT ON TABLE buddy_connections IS 'Group-based buddy-up connections for community building'; COMMENT ON POLICY profiles_select_anon_active ON profiles IS 'Allow anonymous users to view active profiles (required for public_profiles view with security_invoker)'; COMMENT ON VIEW public_roommate_listings IS 'Public view of active roommate listings with security_invoker to respect RLS policies'; COMMENT ON TABLE fairrent_scores IS 'Cached FairRent scores from ML API (v2.3.5+ extended autoregressive model)'; COMMENT ON COLUMN ai_models.settings IS 'Model-specific settings (temperature, topP, etc.)'; COMMENT ON TABLE ai_usage_limits IS 'Subscription tier-based usage limits'; COMMENT ON TABLE expense_categories IS 'Expense categories for shared living'; COMMENT ON TABLE public.report_reasons IS 'Standardized reasons for reporting users, properties, and content with severity levels'; COMMENT ON COLUMN profiles.messaging_mode IS 'User preference for who can message them:
- match_only: Only users with high compatibility (matches)
- open: Anyone can message
- hybrid: Matches can always message, others need approval'; COMMENT ON COLUMN fairrent_scores.confidence IS 'ML model confidence level (0-100)'; COMMENT ON COLUMN ai_routing_rules.priority IS 'Higher values evaluated first (20 > 15 > 10)'; COMMENT ON TABLE calendar_events IS 'User calendar events integrated with bookings and viewings'; COMMENT ON COLUMN messages.reactions IS 'Array of reactions: [{"user_id": "xxx", "emoji": "👍", "created_at": "..."}]'; COMMENT ON COLUMN ai_models.tier_access IS 'Array of subscription tiers that can use this model'; COMMENT ON VIEW rooms_fairrent_ready IS 'View of rooms ready for FairRent scoring with valid required fields, using security_invoker to respect RLS policies'; COMMENT ON COLUMN rooms.fairrent_expires_at IS 'Cache expiry - recalculate if expired (7 days default)'; COMMENT ON TABLE public.community_categories IS 'Community type taxonomy for filtering and organizing communities'; COMMENT ON COLUMN ai_models.model_alias IS 'Human-readable alias (chat-fast, chat-smart, chat-reasoning)'; COMMENT ON VIEW public_roommate_listings_with_profiles IS 'Public view of active roommate listings with profile data, using security_invoker to respect RLS policies'; COMMENT ON TABLE mbti_questions IS 'MBTI personality test questions for flatmate compatibility'; COMMENT ON TABLE ai_config IS 'General platform configuration settings'; COMMENT ON TABLE mbti_personality_types IS 'MBTI personality type descriptions and characteristics'; COMMENT ON TABLE admin_actions IS 'Audit trail for administrative actions (JWT v2 compatible)'; COMMENT ON COLUMN user_compatibility_scores.compatibility_score IS 'Overall compatibility score (0-100) calculated from weighted factors: lifestyle 30%, personality 25%, location 25%, budget 20%'; COMMENT ON COLUMN conversations.match_id IS 'Optional reference to match that created this conversation (for match_bound type)'; COMMENT ON COLUMN buddy_connection_members.invited_by IS 'User who invited this member to the connection'; COMMENT ON COLUMN ai_chat_files.storage_path IS 'Path in storage bucket for file retrieval'; COMMENT ON POLICY rooms_public_read ON rooms IS 'Allows public browsing of rooms in active/available properties'; COMMENT ON COLUMN ai_chats.total_cost IS 'Total cost incurred for this conversation'; COMMENT ON TABLE user_subscriptions IS 'Active user subscriptions to premium plans'; COMMENT ON COLUMN ai_usage_limits.is_active IS 'Whether this usage limit is currently active and should be enforced'; COMMENT ON TABLE feature_flags IS 'Feature flags for A/B testing and gradual rollouts'; COMMENT ON TABLE chat_models IS 'AI chat models available for the platform'; BEGIN; COMMENT ON TABLE property_options IS 'Centralized storage for all form options, filter options, and status/priority values'; COMMENT ON TABLE property_options IS 'Centralized storage for all property-related dropdown options and filters'; COMMENT ON TABLE property_options IS 'Centralized storage for ALL application dropdown options and filters - 100% database-driven platform'; COMMENT ON TABLE property_options IS 'Centralized storage for all form options including persona-specific preferences'; COMMENT ON TABLE property_options IS 'Centralized storage for all form options, filter options, status/priority values, and matching preferences'; COMMIT; COMMENT ON TABLE profiles IS 'Main user profiles table with Clerk integration'; COMMENT ON COLUMN property_options.category IS 'Categories: lease_type, sort_option, property_type, amenity, special_requirement'; COMMENT ON POLICY roommate_listings_public_read ON roommate_listings IS 'Allows public browsing of active roommate listings'; COMMENT ON COLUMN properties.available_from IS 'Date when the property becomes available for move-in. NULL means available immediately or TBD.'; COMMENT ON TABLE mbti_question_answers IS 'Answer options for MBTI questions'; COMMENT ON COLUMN properties.year_built IS 'Year the building was constructed - used for property quality scoring and filtering'; COMMENT ON POLICY properties_select_public ON properties IS 'Allows public browsing of active/available properties, owners see own properties, admins see all'; COMMENT ON COLUMN ai_chats.total_tokens IS 'Total tokens consumed in this conversation'; BEGIN; COMMENT ON COLUMN ai_chat_messages.parts IS 'Rich message content as JSONB array supporting text, images, files, etc.'; COMMENT ON COLUMN ai_chat_messages.parts IS 'Message content as JSONB array supporting text, images, files, etc.'; COMMENT ON COLUMN ai_chat_messages.parts IS 'Message content as JSONB array supporting text, images, files, etc.'; COMMIT; COMMENT ON COLUMN rooms.fairrent_score IS 'FairRent score 0-100 (consolidated from fairrent_room_scores table)'; COMMENT ON COLUMN profiles.notification_preferences IS 'User notification delivery preferences. Controls email, push, in-app notifications and their frequency. Includes per-category toggles.'; COMMENT ON TABLE mass_message_campaigns IS 'Mass messaging campaigns for property managers'; COMMENT ON TABLE amenities IS 'Property amenities reference data'; COMMENT ON TABLE faq_content IS 'Frequently asked questions content'; BEGIN; COMMENT ON TABLE ai_chat_messages IS 'Messages within AI chats supporting rich content via JSONB parts'; COMMENT ON TABLE ai_chat_messages IS 'Messages within AI chats with rich content support via parts JSONB'; COMMENT ON TABLE ai_chat_messages IS 'Messages within AI chats with rich content support via parts JSONB'; COMMIT; COMMENT ON TABLE ai_routing_rules IS 'Rules engine for dynamic model selection based on context'; COMMENT ON TABLE enterprise_settings IS 'White-label configuration for enterprise clients (JWT v2 organization support)'; COMMENT ON TABLE notification_queue IS 'Unified notification delivery system with retry logic'; COMMENT ON TABLE properties IS 'Property listings with comprehensive metadata'; COMMENT ON POLICY property_multimedia_public_read ON property_multimedia IS 'Allows public viewing of images/media for active/available properties'; COMMENT ON COLUMN ai_routing_rules.conditions IS 'JSON conditions: tier, persona, messageLength, etc.'; COMMENT ON TABLE persona_options IS 'User persona/role options for onboarding'; COMMENT ON TABLE platform_settings IS 'Dynamic platform configuration settings manageable via admin interface'; COMMENT ON TABLE property_assignments IS 'Property manager assignments for interested users'; BEGIN; COMMENT ON COLUMN buddy_connection_members.status IS 'Member status: pending, active, declined, inactive'; COMMENT ON COLUMN conversations.status IS 'Conversation status: active, archived, blocked, pending (waiting for acceptance)'; COMMENT ON COLUMN messages.status IS 'Message delivery status: sent, delivered (received by server), read, deleted'; COMMIT; COMMENT ON TABLE ai_usage_tracking IS 'Daily usage tracking per user for cost and limit monitoring'; COMMENT ON COLUMN ai_chat_votes.vote IS 'Vote value: -1 for downvote, 1 for upvote'; COMMENT ON TABLE property_portfolio_items IS 'Junction table linking properties to portfolios with RLS enabled'; COMMENT ON TABLE api_usage_logs IS 'API usage tracking and analytics'; COMMENT ON TABLE email_templates IS 'Email templates for transactional and marketing emails'; COMMENT ON COLUMN user_compatibility_scores.calculation_version IS 'Algorithm version for tracking improvements. Enables selective recalculation when algorithm changes.'; BEGIN; COMMENT ON TABLE ai_chat_files IS 'File attachments for AI chats with storage bucket integration'; COMMENT ON TABLE ai_chat_files IS 'File attachments for AI chats'; COMMENT ON TABLE ai_chat_files IS 'File attachments for AI chats'; COMMIT; COMMENT ON COLUMN profiles.min_compatibility_for_message IS 'Minimum compatibility score (0-100) required to initiate conversation'; COMMENT ON COLUMN ai_chats.model_used IS 'Model alias used for this conversation'; COMMENT ON VIEW properties_fairrent_ready IS 'View of properties ready for FairRent scoring with valid required fields, using security_invoker to respect RLS policies'; COMMENT ON COLUMN fairrent_scores.model_accuracy IS 'Model accuracy at training time (e.g., "99.21%")'; COMMENT ON COLUMN conversations.conversation_type IS 'Type of conversation:
...
- group: Future support for group conversations'; COMMENT ON COLUMN fairrent_scores.data_source IS 'Data source from FairRent API (e.g., "ml_prediction" for v2.3.5+)'; COMMENT ON TABLE verification_flows IS 'Multi-step document verification workflows'; COMMENT ON TABLE error_messages IS 'Translatable error messages for the application'; COMMENT ON TABLE messaging_preferences IS 'User messaging and communication preferences'; COMMENT ON COLUMN profiles.username IS 'User-chosen unique username for public profile URL. Used in settings and public profiles.'; COMMENT ON VIEW properties_search_optimized IS 'Optimized view for property search with room availability, using security_invoker to respect RLS policies'; DO $$
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '═══════════════════════════════════════════════════════════════';
  RAISE NOTICE '✅ UNIFIED MESSAGING SYSTEM MIGRATION COMPLETE';
  RAISE NOTICE '═══════════════════════════════════════════════════════════════';
  RAISE NOTICE '';
  RAISE NOTICE '📋 Changes applied:';
  RAISE NOTICE '  ✓ Dropped old tables: chat_conversations, chat_messages';
  RAISE NOTICE '  ✓ Enhanced conversations table with type, match_id, status';
  RAISE NOTICE '  ✓ Enhanced messages table with status, replies, attachments';
  RAISE NOTICE '  ✓ Added user messaging preferences to profiles';
  RAISE NOTICE '  ✓ Created helper functions for messaging logic';
  RAISE NOTICE '  ✓ Enabled Supabase Realtime for messages & conversations';
  RAISE NOTICE '  ✓ Updated RLS policies for security';
  RAISE NOTICE '';
  RAISE NOTICE '🎯 Features enabled:';
  RAISE NOTICE '  • Match-bound messaging (high compatibility users)';
  RAISE NOTICE '  • Open messaging (anyone can message)';
  RAISE NOTICE '  • Hybrid mode (user-configurable)';
  RAISE NOTICE '  • Real-time message delivery';
  RAISE NOTICE '  • Read receipts';
  RAISE NOTICE '  • Typing indicators';
  RAISE NOTICE '  • Message threading (replies)';
  RAISE NOTICE '  • File attachments';
  RAISE NOTICE '  • Message reactions';
  RAISE NOTICE '';
  RAISE NOTICE '💡 Next steps:';
  RAISE NOTICE '  1. Update API endpoints to use new schema';
  RAISE NOTICE '  2. Update client-side hooks for Realtime';
  RAISE NOTICE '  3. Add messaging preferences UI';
  RAISE NOTICE '  4. Test real-time functionality';
  RAISE NOTICE '';
  RAISE NOTICE '═══════════════════════════════════════════════════════════════';
END $$