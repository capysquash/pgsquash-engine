...
BEGIN
  ...
  SELECT 
    ...
    END as status;
END $$; CREATE OR REPLACE FUNCTION public.log_admin_action(p_admin_user_id text, p_admin_email text, p_action_type text, p_resource_type text, p_resource_id text = NULL, p_action_details jsonb = '{}', p_success boolean = true, p_error_message text = NULL) RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER VOLATILE AS $$
declare
    log_id uuid;
begin
...
BEGIN
    ...
    END IF;
END;
$$; CREATE TRIGGER update_conversations_updated_at BEFORE UPDATE ON public.conversations FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_tasks_updated_at BEFORE UPDATE ON public.tasks FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_memory_card_timestamp_trigger BEFORE UPDATE ON public.memory_cards FOR EACH ROW WHEN (old.access_count IS DISTINCT FROM new.access_count) EXECUTE FUNCTION public.update_memory_card_timestamp(); CREATE TRIGGER update_subtasks_updated_at BEFORE UPDATE ON public.subtasks FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_planning_sessions_updated_at BEFORE UPDATE ON public.planning_sessions FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_task_templates_updated_at BEFORE UPDATE ON public.task_templates FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_memory_cards_updated_at BEFORE UPDATE ON public.memory_cards FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_planning_preferences_updated_at BEFORE UPDATE ON public.planning_preferences FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_user_profiles_updated_at BEFORE UPDATE ON public.user_profiles FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_memory_search_vector_trigger BEFORE INSERT OR UPDATE ON public.memory_cards FOR EACH ROW EXECUTE FUNCTION public.update_memory_card_search_vector(); CREATE TRIGGER update_task_categories_updated_at BEFORE UPDATE ON public.task_categories FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column(); CREATE TRIGGER update_memory_relevance_trigger BEFORE UPDATE ON public.memory_cards FOR EACH ROW EXECUTE FUNCTION public.update_memory_card_relevance(); CREATE TRIGGER auto_complete_task_on_subtasks AFTER UPDATE ON public.subtasks FOR EACH ROW EXECUTE FUNCTION check_task_completion(); CREATE INDEX IF NOT EXISTS idx_recovery_sessions_start_time ON public.recovery_sessions USING btree (start_time); CREATE INDEX IF NOT EXISTS idx_user_profiles_stress_level ON public.user_profiles USING btree (current_stress_level); CREATE INDEX IF NOT EXISTS idx_system_monitoring_recent ON public.system_monitoring USING btree ("timestamp" DESC); CREATE INDEX IF NOT EXISTS idx_social_interactions_interaction_type ON public.social_interactions USING btree (interaction_type); CREATE INDEX IF NOT EXISTS idx_crisis_baselines_user_id ON public.crisis_baselines USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_task_categories_user_id ON public.task_categories USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_user_profiles_burnout_risk ON public.user_profiles USING btree (burnout_risk_score) WHERE burnout_risk_score > 50; CREATE INDEX IF NOT EXISTS idx_behavioral_patterns_session ON public.behavioral_patterns USING btree (session_id); CREATE INDEX IF NOT EXISTS idx_system_settings_public ON public.system_settings USING btree (is_public) WHERE is_public = true; CREATE INDEX IF NOT EXISTS idx_tasks_deadline ON public.tasks USING btree (deadline); CREATE INDEX IF NOT EXISTS idx_memory_cards_user_ui_states ON public.memory_cards USING gin (surfacing_ui_states); CREATE INDEX IF NOT EXISTS idx_admin_action_logs_timestamp ON public.admin_action_logs USING btree ("timestamp"); CREATE INDEX IF NOT EXISTS idx_admin_action_logs_admin ON public.admin_action_logs USING btree (admin_user_id); CREATE INDEX IF NOT EXISTS idx_system_analytics_date ON public.system_analytics USING btree (date); CREATE INDEX IF NOT EXISTS idx_user_profiles_onboarding_completed ON public.user_profiles USING btree (onboarding_completed); CREATE INDEX IF NOT EXISTS idx_memory_cards_updated_at ON public.memory_cards USING btree (updated_at DESC); CREATE INDEX IF NOT EXISTS idx_memory_cards_search_vector ON public.memory_cards USING gin (search_vector); CREATE INDEX IF NOT EXISTS idx_energy_patterns_user_id ON public.energy_patterns USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_social_interactions_user_id ON public.social_interactions USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_subtasks_task_id ON public.subtasks USING btree (task_id); CREATE INDEX IF NOT EXISTS idx_tasks_created_at ON public.tasks USING btree (created_at); CREATE INDEX IF NOT EXISTS idx_planning_sessions_start_time ON public.planning_sessions USING btree (start_time); CREATE INDEX IF NOT EXISTS idx_user_profiles_primary_support ON public.user_profiles USING btree (primary_support); CREATE INDEX IF NOT EXISTS idx_tasks_completed ON public.tasks USING btree (completed); CREATE INDEX IF NOT EXISTS idx_task_templates_user_id ON public.task_templates USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_recovery_sessions_quality ON public.recovery_sessions USING btree (quality); CREATE INDEX IF NOT EXISTS idx_behavioral_patterns_timestamp ON public.behavioral_patterns USING btree ("timestamp"); CREATE INDEX IF NOT EXISTS idx_user_profiles_clerk_id ON public.user_profiles USING btree (clerk_user_id); CREATE INDEX IF NOT EXISTS idx_subtasks_order ON public.subtasks USING btree (task_id, order_position); CREATE INDEX IF NOT EXISTS idx_system_settings_key ON public.system_settings USING btree (setting_key); CREATE INDEX IF NOT EXISTS idx_behavioral_patterns_user_id ON public.behavioral_patterns USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_crisis_events_detection_time ON public.crisis_events USING btree (detection_timestamp); CREATE INDEX IF NOT EXISTS idx_system_analytics_period ON public.system_analytics USING btree (period_type); CREATE INDEX IF NOT EXISTS idx_planning_sessions_active ON public.planning_sessions USING btree (user_id, end_time) WHERE end_time IS NULL; CREATE INDEX IF NOT EXISTS idx_social_battery_user_id ON public.social_battery USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_crisis_logs_user_id ON public.crisis_support_logs USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_tasks_priority ON public.tasks USING btree (priority); CREATE INDEX IF NOT EXISTS idx_user_profiles_ui_state ON public.user_profiles USING btree (ui_state); CREATE INDEX IF NOT EXISTS idx_energy_patterns_activity ON public.energy_patterns USING btree (activity_type); CREATE INDEX IF NOT EXISTS idx_user_profiles_crisis_mode ON public.user_profiles USING btree (crisis_mode_active) WHERE crisis_mode_active = true; CREATE INDEX IF NOT EXISTS idx_behavioral_patterns_type ON public.behavioral_patterns USING btree (interaction_type); CREATE INDEX IF NOT EXISTS idx_social_interactions_started_at ON public.social_interactions USING btree (started_at); CREATE INDEX IF NOT EXISTS idx_tasks_user_id ON public.tasks USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_user_activity_snapshots_active ON public.user_activity_snapshots USING btree (is_active); CREATE INDEX IF NOT EXISTS idx_system_settings_category ON public.system_settings USING btree (category); CREATE INDEX IF NOT EXISTS idx_admin_action_logs_action_type ON public.admin_action_logs USING btree (action_type); CREATE INDEX IF NOT EXISTS idx_recovery_sessions_user_id ON public.recovery_sessions USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_user_activity_snapshots_user_id ON public.user_activity_snapshots USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_recovery_activities_active ON public.recovery_activities USING btree (is_active); CREATE INDEX IF NOT EXISTS idx_admin_action_logs_resource ON public.admin_action_logs USING btree (resource_type, resource_id); CREATE INDEX IF NOT EXISTS idx_crisis_logs_timestamp ON public.crisis_support_logs USING btree (crisis_started); CREATE INDEX IF NOT EXISTS idx_memory_cards_user_updated ON public.memory_cards USING btree (user_id, updated_at DESC); CREATE INDEX IF NOT EXISTS idx_user_activity_snapshots_date ON public.user_activity_snapshots USING btree (snapshot_date); CREATE INDEX IF NOT EXISTS idx_crisis_events_severity ON public.crisis_events USING btree (severity_level); CREATE INDEX IF NOT EXISTS idx_tasks_category ON public.tasks USING btree (category); CREATE INDEX IF NOT EXISTS idx_recovery_sessions_activity_id ON public.recovery_sessions USING btree (activity_id); CREATE INDEX IF NOT EXISTS idx_user_profiles_energy_level ON public.user_profiles USING btree (energy_level); CREATE INDEX IF NOT EXISTS idx_recovery_plans_active ON public.recovery_plans USING btree (is_active); CREATE INDEX IF NOT EXISTS idx_user_activity_snapshots_attention ON public.user_activity_snapshots USING btree (requires_admin_attention) WHERE requires_admin_attention = true; CREATE INDEX IF NOT EXISTS idx_memory_cards_user_energy ON public.memory_cards USING gin (surfacing_energy_states); CREATE INDEX IF NOT EXISTS idx_usage_analytics_date ON public.usage_analytics USING btree (date); CREATE INDEX IF NOT EXISTS idx_memory_cards_tags ON public.memory_cards USING gin (tags); CREATE INDEX IF NOT EXISTS idx_crisis_events_user_id ON public.crisis_events USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_social_battery_timestamp ON public.social_battery USING btree ("timestamp"); CREATE INDEX IF NOT EXISTS idx_conversations_conversation_id ON public.conversations USING btree (conversation_id); CREATE INDEX IF NOT EXISTS idx_planning_sessions_user_id ON public.planning_sessions USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_memory_cards_user_contexts ON public.memory_cards USING gin (surfacing_contexts); CREATE INDEX IF NOT EXISTS idx_crisis_events_unresolved ON public.crisis_events USING btree (resolution_timestamp) WHERE resolution_timestamp IS NULL; CREATE INDEX IF NOT EXISTS idx_memory_cards_keywords ON public.memory_cards USING gin (surfacing_keywords); CREATE INDEX IF NOT EXISTS idx_conversations_timestamp ON public.conversations USING btree ("timestamp"); CREATE INDEX IF NOT EXISTS idx_recovery_preferences_user_id ON public.recovery_preferences USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_conversations_updated_at ON public.conversations USING btree (updated_at DESC); CREATE INDEX IF NOT EXISTS idx_memory_cards_category ON public.memory_cards USING btree (category); CREATE INDEX IF NOT EXISTS idx_recovery_activities_difficulty ON public.recovery_activities USING btree (difficulty_level); CREATE INDEX IF NOT EXISTS idx_recovery_plans_user_id ON public.recovery_plans USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_system_monitoring_timestamp ON public.system_monitoring USING btree ("timestamp"); CREATE INDEX IF NOT EXISTS idx_conversations_user_id ON public.conversations USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_memory_cards_importance ON public.memory_cards USING btree (importance); CREATE INDEX IF NOT EXISTS idx_memory_cards_user_id ON public.memory_cards USING btree (user_id); CREATE INDEX IF NOT EXISTS idx_recovery_activities_type ON public.recovery_activities USING btree (type); CREATE INDEX IF NOT EXISTS idx_energy_patterns_timestamp ON public.energy_patterns USING btree ("timestamp"); CREATE INDEX IF NOT EXISTS idx_subtasks_completed ON public.subtasks USING btree (completed); CREATE POLICY admin_system_settings_jwt_v2 ON public.system_settings TO authenticated USING ((current_clerk_org_role() IN ('admin', 'owner') OR (auth.jwt() ->> 'role') = 'service_role') AND validate_jwt_version()) ; CREATE POLICY crisis_logs_jwt_v2 ON public.crisis_support_logs TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY admin_user_snapshots_jwt_v2 ON public.user_activity_snapshots TO authenticated USING ((current_clerk_org_role() IN ('admin', 'owner') OR (auth.jwt() ->> 'role') = 'service_role') AND validate_jwt_version()) ; CREATE POLICY admin_monitoring_data_jwt_v2 ON public.system_monitoring TO authenticated USING ((current_clerk_org_role() IN ('admin', 'owner') OR (auth.jwt() ->> 'role') = 'service_role') AND validate_jwt_version()) ; CREATE POLICY user_profiles_jwt_v2 ON public.user_profiles TO authenticated USING (clerk_user_id = current_clerk_user_id() AND validate_jwt_version()) ; CREATE POLICY public_settings_readable_jwt_v2 ON public.system_settings FOR SELECT TO authenticated USING (is_public = true AND validate_jwt_version()) ; CREATE POLICY ai_providers_service_only ON public.ai_providers TO authenticated USING ((auth.jwt() ->> 'role') = 'service_role' AND validate_jwt_version()) ; CREATE POLICY task_categories_user_access_jwt_v2 ON public.task_categories TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY energy_patterns_jwt_v2 ON public.energy_patterns TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY memory_cards_jwt_v2 ON public.memory_cards TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY recovery_activities_admin_modify ON public.recovery_activities FOR INSERT TO authenticated WITH CHECK ((auth.jwt() ->> 'role') = 'service_role' AND validate_jwt_version()) ; CREATE POLICY planning_preferences_user_access_jwt_v2 ON public.planning_preferences TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY crisis_events_jwt_v2 ON public.crisis_events TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY admin_action_logs_jwt_v2 ON public.admin_action_logs TO authenticated USING ((current_clerk_org_role() IN ('admin', 'owner') OR (auth.jwt() ->> 'role') = 'service_role') AND validate_jwt_version()) ; CREATE POLICY recovery_activities_admin_update ON public.recovery_activities FOR UPDATE TO authenticated USING ((auth.jwt() ->> 'role') = 'service_role' AND validate_jwt_version()) WITH CHECK ((auth.jwt() ->> 'role') = 'service_role' AND validate_jwt_version()) ; CREATE POLICY social_battery_jwt_v2 ON public.social_battery TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY usage_analytics_service_only ON public.usage_analytics TO authenticated USING ((auth.jwt() ->> 'role') = 'service_role' AND validate_jwt_version()) ; CREATE POLICY planning_sessions_user_access_jwt_v2 ON public.planning_sessions TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY recovery_sessions_jwt_v2 ON public.recovery_sessions TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY tasks_user_access_jwt_v2 ON public.tasks TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY recovery_preferences_jwt_v2 ON public.recovery_preferences TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY conversations_jwt_v2 ON public.conversations TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY behavioral_patterns_jwt_v2 ON public.behavioral_patterns TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY social_interactions_jwt_v2 ON public.social_interactions TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY subtasks_user_access_jwt_v2 ON public.subtasks TO authenticated USING (task_id IN (SELECT t.id FROM public.tasks t JOIN public.user_profiles up ON t.user_id = up.id WHERE up.clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY recovery_plans_jwt_v2 ON public.recovery_plans TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY task_templates_user_access_jwt_v2 ON public.task_templates TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY crisis_baselines_jwt_v2 ON public.crisis_baselines TO authenticated USING (user_id IN (SELECT id FROM public.user_profiles WHERE clerk_user_id = current_clerk_user_id()) AND validate_jwt_version()) ; CREATE POLICY recovery_activities_admin_delete ON public.recovery_activities FOR DELETE TO authenticated USING ((auth.jwt() ->> 'role') = 'service_role' AND validate_jwt_version()) ; CREATE POLICY admin_analytics_jwt_v2 ON public.system_analytics TO authenticated USING ((current_clerk_org_role() IN ('admin', 'owner') OR (auth.jwt() ->> 'role') = 'service_role') AND validate_jwt_version()) ; CREATE POLICY recovery_activities_read_all ON public.recovery_activities FOR SELECT TO authenticated USING (validate_jwt_version()) 